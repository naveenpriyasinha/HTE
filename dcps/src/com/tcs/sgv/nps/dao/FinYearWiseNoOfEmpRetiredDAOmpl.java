...
public class FinYearWiseNoOfEmpRetiredDAOmpl extends GenericDaoHibernateImpl{ 
	...
	public FinYearWiseNoOfEmpRetiredDAOmpl(Class type, SessionFactory sessionFactory) {
	...
	public List getNoOfEmpRetiredReport(String lStrLocCode, String ddocode) {
		...
		lSBQuery.append("SELECT FIN.FIN_YEAR_DESC,NVL(COUNT(DISTINCT (CASE WHEN EMP.DCPS_OR_GPF = 'Y' THEN EMP.DCPS_EMP_ID END)),0) AS NEW_PENSION_SCHEME,");
		lSBQuery.append("NVL(COUNT(DISTINCT (CASE WHEN EMP.DCPS_OR_GPF = 'N' and  EMP.PAYSCALE <> 101  THEN EMP.DCPS_EMP_ID END)),0) AS OLD_PENSION_SCHEME FROM IFMS.MST_DCPS_EMP EMP ");
		...
		lSBQuery.append("JOIN IFMS.DB_LEVEL_FIN_YEAR_MST FIN ON head.PAYBILL_YEAR=FIN.YEAR AND head.PAYBILL_MONTH=FIN.MONTH ");
		lSBQuery.append("WHERE head.PAYBILL_YEAR in (2023) and head.paybill_month=3 and head.APPROVE_FLAG in (0,1) ");
		lSBQuery.append("AND MAP.REPT_DDO_CODE <> '"+ddocode+"' and bill.emp_id is not null GROUP BY FIN.FIN_YEAR_DESC");
