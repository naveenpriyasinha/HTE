...
public class LedgerReportDAOImpl extends GenericDaoHibernateImpl{
	...
	public List getbillGroupList(String ddoCode) {
		...
		logger.error("sql query for getBilldetailsList is"+sql1query.toString());
		logger.error("sql1query.list()"+sql1query.list().size());
		logger.info("sql query for getBilldetailsList is"+sql1query.toString());
		logger.info("sql1query.list()"+sql1query.list().size());
		return sql1query.list();
	}
...
*/
	...
	public void updateNewBasicPay(String strSevarthId, Long newbasicPaySvnPC,Date effectDate,String levelId) {		
		...
		logger.info("updateNewBasicPay  LedgerReportDAOImpl");
		Session hibSession =getSession();
		StringBuffer str = new StringBuffer();	
	...
	public int chkPostAndSerExp(String strSevarthId, long locId,String flag) {
		...
		queryBuffer.append(" inner join ORG_DDO_MST ddo on ddo.DDO_CODE = emp.DDO_CODE where emp.SEVARTH_ID = '"+strSevarthId+"' and (emp.EMP_SERVEND_DT >= sysdate or emp.EMP_SERVEND_DT is null) ");	
		else
		...
		queryBuffer.append(" inner join ORG_DDO_MST ddo on ddo.DDO_CODE = emp.DDO_CODE where emp.emp_name = '"+strSevarthId+"' and (emp.EMP_SERVEND_DT >= sysdate or emp.EMP_SERVEND_DT is null) ");
		
		Query query = hibSession.createSQLQuery(queryBuffer.toString());
		logger.info("query is chkPostAndSerExp*************"+query.toString());
		String status = query.uniqueResult().toString();
		int duplicatePayslipIDCount = Integer.parseInt(status);
		logger.error("duplicatePayslipIDCount" + duplicatePayslipIDCount);
		return duplicatePayslipIDCount;
	...
	*/
	...
	// added by lekhchand 	 TA 2022 
		...
		public List getNPSEmpMpgList1(long eisEmpId, int yearGiven, int monthGiven) {
			// TODO Auto-generated method stub
				Session hibSession = getSession();
				StringBuffer sb = new StringBuffer();
				sb.append(" SELECT * FROM HR_EIS_EMP_COMPONENT_GRP_MST mst inner join HR_EIS_EMP_COMPONENT_MPG emp on mst.EMP_COMPO_GRP_ID = emp.COMPO_GROUP_ID  ");
				...
				sb.append(" inner join mst_dcps_emp dcps  on dcps.ORG_EMP_MST_ID = eis.EMP_MPG_ID and dcps.REG_STATUS = 1  ");
				sb.append(" where mst.EMP_ID = "+eisEmpId+" and mst.IS_ACTIVE = 1  ");
				//sb.append(" and emp.IS_ACTIVE = 1  and emp.COMPO_ID in (555,666,59) and to_char(dcps.DOJ,'mm-YYYY') <> to_char('"+monthGiven+"-"+yearGiven+"') ");		
				if( monthGiven < 10)
				{
				sb.append(" and emp.IS_ACTIVE = 1  and emp.COMPO_ID in (555,666,59) and to_char(dcps.DOJ,'mm-YYYY')  <> to_char('0"+monthGiven+"-"+yearGiven+"') ");
				}
				else
				{
					sb.append(" and emp.IS_ACTIVE = 1  and emp.COMPO_ID in (555,666,59) and to_char(dcps.DOJ,'mm-YYYY') <> to_char('"+monthGiven+"-"+yearGiven+"') ");
		  ...
		  public String getCityCatforTA(long empId) {
			    String TaCatCityClass = null;
			    Session hibSession = getSession();
			    StringBuffer queryBuffer = new StringBuffer();
			    queryBuffer
			      .append("SELECT mstddo.CITY_CLASS_FOR_TA FROM MST_DCPS_EMP mstdcpsemp inner JOIN  HR_EIS_EMP_MST hreisemp ON  mstdcpsemp.ORG_EMP_MST_ID  =hreisemp.EMP_MPG_ID inner Join MST_DCPS_DDO_OFFICE  mstddo  On mstdcpsemp.DDO_CODE =mstddo.DDO_CODE  where  hreisemp.EMP_ID=" + 
			        empId);
			    Query sQLQuery = hibSession.createSQLQuery(queryBuffer.toString());
			    this.logger.error("Query check level  to get getCityCatforTA :: " + 
			        queryBuffer.toString());
			    if (sQLQuery.uniqueResult() != null)
			      TaCatCityClass = sQLQuery.uniqueResult().toString(); 
			    return TaCatCityClass;
			  }
			...
			public Long getSvnPCLevel2(long empId) {
		        ...
		        StringBuffer queryBuffer = new StringBuffer();
		        queryBuffer
		                .append("SELECT rltgrade.LEVEL  FROM MST_DCPS_EMP mstdcpsemp inner JOIN  HR_EIS_EMP_MST hreisemp ON  mstdcpsemp.ORG_EMP_MST_ID =hreisemp.EMP_MPG_ID  inner join RLT_PAYBAND_GP_STATE_7PC rltgrade ON mstdcpsemp.GRADE_PAY=rltgrade.GRADE_PAY   where hreisemp.EMP_ID= "
