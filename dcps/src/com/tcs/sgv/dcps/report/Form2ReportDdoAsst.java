	...
	public Collection findReportData(ReportVO report, Object criteria) throws ReportException {
		...
		try {
			...
			if (report.getReportCode().equals("700081")) {
				...
				lSBQuery.append(" SELECT EM.dcps_id,EM.emp_name,HES.emp_id,EM.dcps_emp_id,HPP.PO, HPP.D_PAY, HPP.DA, HPP.DCPS, EM.dcps_emp_id,HPP.SVNPC_DA");
				lSBQuery.append(" ,HPP.NPS_EMPLR-(SELECT case when count(*)>=1 then sum(nvl(NPS_EMPLR_CONTRI_DED,0)) else 0 end FROM ifms.TRN_DCPS_CONTRIBUTION  as trn inner join ifms.SGVC_FIN_YEAR_MST as fin ");
				lSBQuery.append(" on fin.FIN_YEAR_ID=trn.FIN_YEAR_ID   where BILL_GROUP_ID=:billGroupId and FIN_YEAR_CODE= :year and MONTH_ID=:month  and ");
				lSBQuery.append(" trn.TYPE_OF_PAYMENT<>'700046' and trn.DCPS_EMP_ID=EM.dcps_emp_id),HPP.NPS_EMPLR_CONTRI_DED FROM HR_PAY_PAYBILL HPP");
				...
				gLogger.info("lStrYearCode:::"+lStrYearCode);

				gLogger.info("lStrBillGroupId:::"+lStrBillGroupId);

				gLogger.info("Query 07/12/2020:::"+lSBQuery.toString());
				...
				Double lDoubleContributionNps = 0d;
				Double SVNPC_DA = 0d;
				...
				if (lLstFinal != null && !lLstFinal.isEmpty()) {
					...
					while (it.hasNext()) {
						...
						if(!BrokenPeriodFlagForEmployee)
						{
							...
							if (tuple[10] != null) // Contribution Under Tier-1 14% of (Basic + DP + DA) Rs.	
							{
								td.add(Round(Double.parseDouble(tuple[10].toString()), 0));
								lDoubleTotalContributionNps = Double.parseDouble(tuple[10].toString()) + lDoubleTotalContributionNps;
							} else {
								td.add("");
							}
							td.add(""); // Remarks
								...
								if(lListBrokenIdPKListForEmp.size() != 0)
								{
									for(Integer lIntCntBrokenIdPk = 0 ;lIntCntBrokenIdPk < lListBrokenIdPKListForEmp.size() ; lIntCntBrokenIdPk++)
									{
										if(lListBrokenIdPKListForEmp.get(lIntCntBrokenIdPk) != null)
										{
											...
											lDoubleContributionNps = Double.parseDouble(lObjOfflineContriDAO.getDCPSValueForBrokenPeriodNPS(lLongBrokenIdPk).get(0).toString().trim());
...
/*											if (tuple[6] != null) // DA
											...
											if (tuple[10] != null) // Contribution Under Tier-1 14% of (Basic + DP + DA) Rs.	
											{
												td.add(Round(lDoubleContributionNps,0));
												//td.add(Round(Double.parseDouble(tuple[10].toString()), 0));
												lDoubleTotalContributionNps = lDoubleContributionNps + lDoubleTotalContributionNps;
											} else {
												td.add("");
											}
				...
				lArrReportColumnVOs[10].setColumnWidth(20);
				lArrReportColumnVOs[11].setColumnHeader("");
				lArrReportColumnVOs[11].setColumnName("");
				lArrReportColumnVOs[11].setColumnWidth(9);
				...
				td.add(lDoubleTotalContribution+lDoubleTotalContributionNps);
				td.add("" + space(1));
				td.add("" + space(1));
				dataListForTableWithHiddenColumns.add(td);
				...
				rowList = new ArrayList();
				rowList.add(new StyledData("Total Amount Ruppes: " + (lDoubleTotalContribution+lDoubleTotalContributionNps) + "/-", rowsFontsVO));
				dataList.add(rowList);

				String lStrTotAmtInWords = EnglishDecimalFormat.convertWithSpace(new BigDecimal(lDoubleTotalContribution+lDoubleTotalContribution));
