  			...
  			if(eval(obj) == 14) {
			   	...
			   	for(var i=0;i<UserEntries.length;i++)
				{
					...
					if(UserEntries.options[i].selected)
					{
					    ...
					    x.innerHTML=document.form1.userId.value;
					    y.innerHTML=document.form1.UserName.value;
					    z.innerHTML=tempCombo;
					    var editString = "<a style=\"cursor:hand\" onClick=\"getSelected(this,"+obj+", '"+UserEntries.options[i].value+"' , '"+UserEntries.options[i].text+"' , '"+document.form1.userId.value+"' , '"+document.form1.UserName.value+"')\"> Revoke </a>";
