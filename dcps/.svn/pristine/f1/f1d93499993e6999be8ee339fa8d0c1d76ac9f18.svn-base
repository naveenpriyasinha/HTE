...
<fmt:setBundle basename="resources.pensionpay.PensionCaseConstants" var="AGFileConstants" scope="request"/>
...
FILE_CPO_ALLOC='<fmt:message bundle="${AGFileConstants}" key="FILE_CPO_ALLOC" />';
FILE_CPO='<fmt:message bundle="${AGFileConstants}" key="FILE_CPO" />';
FILE_FAMILY='<fmt:message bundle="${AGFileConstants}" key="FILE_FAMILY" />';
FILE_GPO_ALLOC='<fmt:message bundle="${AGFileConstants}" key="FILE_GPO_ALLOC" />';
FILE_PPO_ALLOC='<fmt:message bundle="${AGFileConstants}" key="FILE_PPO_ALLOC" />';
FILE_PENSIONER='<fmt:message bundle="${AGFileConstants}" key="FILE_PENSIONER" />';
FILE_RECOVERY='<fmt:message bundle="${AGFileConstants}" key="FILE_RECOVERY" />';
</script>
<style type="text/css">
  .removeLink { 
...
function killUpdate<%=request.getParameter("attachmentName")%>(message)
{
    ...
    if(message != '')
    {
 	 ...
 	 // var obj = document.forms[formNameGlobalVar<%=request.getParameter("attachmentName")%>].desc<%=request.getParameter("attachmentName")%>;
 	  ...
 	  //**********Commenting code to not clear description field , to set it from validateFile method :: added by shripal*********************************//
...
{		
	...
	catch(e)
	{
		...
		if(document.forms[formNameGlobalVar<%=request.getParameter("attachmentName")%>].desc<%=request.getParameter("attachmentName")%>.value != "")
		{
			...
			//********** change for setting correct file description starts :: added by shripal <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
			// col1<%=request.getParameter("attachmentName")%>.innerHTML= fileDescription;
			col1<%=request.getParameter("attachmentName")%>.innerHTML= arrFileNames[0];
			arrFileNames.splice(0,1); 
			if(arrFileNames.length > 0)
			{
				document.getElementById("descscan").value = arrFileNames[0];
			}
			else
			{
				document.getElementById("descscan").value = "";
			}
			// change for setting correct file description ends :: added by shripal >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
	...
	if(confirm('<fmt:message bundle="${AttachmentLables}" key="REMOVE_ALTER_MSG"/>'))
	{
	  document.getElementById("hidFileName").value="";
		...
		try
		{
			...
			//*********Code to get file description of the row that is being deleted , starts :: added by shripal<<<<<<<<<<<<<<<<<<<<<<<<
			var lTable = document.getElementById("myTable<%=request.getParameter("attachmentName")%>").childNodes;
			var lTrOfTable = lTable[0].childNodes;
			var lTdOfTr = lTrOfTable[rownum+1].childNodes;
			//var rowCnt = row<%=request.getParameter("attachmentName")%>.rowIndex;
			arrFileNames[arrFileNames.length] = lTdOfTr[0].innerHTML;
			document.getElementById("descscan").value = arrFileNames[0];
			//*********Code to get file description of the row that is being deleted Ends >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
...
function viewRow<%=request.getParameter("attachmentName")%>(src,rowNumber)
{
	...
	try
	{
		...
		//alert("url for view is :"+urlValue);
...
<%}%>
 <td colspan="3" align="center">
	 <c:set var="submitName" value='<%=request.getParameter("attachmentName")%>'></c:set>
		 <hdiits:button type="button" name="submitButton${submitName}" id="submitButton${submitName}" classcss="bigbutton" captionid="ADD_FILE" bundle="${AttachmentLables}" onclick="validateFile${submitName}('${formName}');" />
 <%
...
<hdiits:jsField jsFunction='checkAttachmentValue("${attachmentUniqueName}", "${errMessage}")' name="checkAttachmentValue_${attachmentUniqueName}" />
	...
	<script type="text/javascript">
		// Array of list of file Names of AG files :: added by shripal
	  	// change file names in this array as provided by AG...
	  	var arrFileNames = new Array(FILE_CPO_ALLOC,FILE_CPO,FILE_FAMILY,FILE_GPO_ALLOC,FILE_PENSIONER,FILE_PPO_ALLOC,FILE_RECOVERY);
	  	//alert("array is :"+arrFileNames);
		document.getElementById("descscan").value = arrFileNames[0];
		document.getElementById("descscan").disabled = true;
		// Function to check whether file browsed is of file description type or not
		function validateFile<%=request.getParameter("attachmentName")%>(formName)
		{
			var uploadedFileName = document.forms[formName].importFile<%=request.getParameter("attachmentName")%>.value;
			var typeOfFileArr = uploadedFileName.split("\\");
			var typeOfFilewithExtn = typeOfFileArr[(typeOfFileArr.length) - 1];
			var typeOfFile = (typeOfFilewithExtn.split("."))[0];
			if(arrFileNames[0] == typeOfFile)
			{
				startStatusCheck${submitName}('${formName}');
			}
			else
			{
				alert("Please attach proper file same as shown in file description");
			}
			document.getElementById("descscan").value = arrFileNames[0];
		}
