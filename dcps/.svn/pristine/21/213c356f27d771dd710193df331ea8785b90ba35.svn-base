...
<c:set var="LocationList" value="${resValue.LocationList}"></c:set>
<c:set var="CorrVO" value="${resValue.FmsCorrMst}"></c:set>
<c:set var="LocationVO" value="${resValue.CmnLocationMst}"></c:set>
<c:set var="LookupVO" value="${resValue.CmnLookupMst}"></c:set>
<c:set var="CorrespondenceTypeList" value="${resValue.CorrespondenceTypeList}"></c:set>
<c:set var="OrgPostMstbyCrtPost" value="${resValue.OrgPostMstbyCrtPost}"></c:set>
<c:set var="LoginUserPost" value="${resValue.PostDetailsList}"></c:set>
<c:set var="CmnLocationMstForCrtLoc" value="${resValue.CmnLocationMstForCrtLoc}"></c:set>
<c:set var="LoginUserLoc" value="${resValue.LoginUserLoc}"></c:set>
<c:set var="BranchlocMpgList" value="${resValue.CmnBranchlocMpgList}"></c:set>
...
<hdiits:form name="EditcorrespondenceForm" method="POST" action="./hdiits.htm" encType="multipart/form-data" validate="true"  >

<hdiits:hidden name="DDTextDecisionFlag" id="DDTextDecisionFlag_Id" default="Text"/>
<hdiits:hidden name="flagFromWorkList" default="workList"/>
<c:if test="${resValue.SessionAttribFlag ne 'Y'}">
<script>
	document.getElementById('DDTextDecisionFlag_Id').value="DD";
</script>
	<hdiits:hidden name="tabcnt" default="nill"/>
	<hdiits:hidden name="pk_FrmCorrMst" default="${CorrVO.corrId}"/>
</c:if>
<div id="tabmenu">
	<ul id="maintab" class="shadetabs">
	<c:choose>
		<c:when test="${empty resValue.titleFlag}">
	           		<li class="selected"><a href="#" rel="tcontent1"><hdiits:caption captionid="WF.EditCorr" bundle="${fmsLables}"/></a></li>
	     </c:when>
	     <c:otherwise>
		...
		<table  border="0">
			<tr>
				...
				<td width="16.6%" align="left">		
			  		<hdiits:caption captionid="WF.LOCNAME" bundle="${fmsLables}"/>
				</td>
				<td  width="16.6%" align="left">
				<c:choose>
					<c:when test="${resValue.SessionAttribFlag eq 'Y'}">
					...
					<c:otherwise>	
						<hdiits:text name="userLocFromFile" default="${CmnLocationMstForCrtLoc.locName}" readonly="true"/>
					</c:otherwise>
				</c:choose>
				</td>
			...
			<tr>
			<!-- section -->
			 <td width="16.6%" align="left">		<hdiits:caption captionid="WF.Sec" bundle="${fmsLables}"/>
					</td>
						<td width="16.6%" align="left">
							<c:choose>
								<c:when test="${resValue.SessionAttribFlag eq 'Y'}">
								...
								<c:otherwise>
									<hdiits:text name="sectionFromFile" default="${resValue.branchName}" readonly="true"/>
								</c:otherwise>
							</c:choose>
						</td>
						
					<!-- subject -->	
						<td width="16.6%" align="left">		<hdiits:caption captionid="WF.BrSub" bundle="${fmsLables}"/>
						</td>
						<td width="16.6%" align="left">
							<c:choose>
								<c:when test="${resValue.SessionAttribFlag eq 'Y'}">
									<hdiits:select id="dd_brSub" name="brSub" mandatory="true" validation="sel.isrequired" captionid="WF.BrSub" bundle="${fmsLables}" condition="checkLoginUserRole()" onchange="fetchCategories('fromDoc')">
								...
								<c:otherwise>
									<hdiits:text name="brSubFromFile" default="${resValue.DocName}" readonly="true"/>
								</c:otherwise>
							</c:choose>
						</td>
			...
			<tr>
				...
				<td width="25%" align="left">
				<hdiits:text name="refNo" size="31" default="${CorrVO.corrRefNo}"/>
				</td>
			...
			<tr>
				...
				<table width="100%">
					<tr>
							...
							<td  align="left">
								<hdiits:textarea  name="lettersubDesc" cols="85" maxlength="128" default="${CorrVO.corrDesc}"/>
			...
			<tr>
				...
				<table width="100%">
					<tr>
								...
								<td   align="left">
									<hdiits:textarea  name="notings" cols="85" maxlength="128" default="${CorrVO.notingDesc}"/>
			...
			<tr>
				<td colspan="6">
					...
					<tr>
						...
						<td  align="left">
								<hdiits:textarea  name="enclousure" cols="85" maxlength="128" default="${CorrVO.enclosureDesc}"/>
			...
			<tr><td align="center" colspan="4"><font size="3" color="gray"><b><fmt:message key="WF.SenderDtl"  bundle="${fmsLables}"></fmt:message></b></font></td></tr>
				<tr>
						...
						<td  width="25%" align="left">
							<hdiits:text name="recFrom" size="31" captionid="WF.RecFrom" bundle="${fmsLables}" default="${CorrVO.recvdFrm}"/>
						</td>
				</tr>
				<tr>		
						<td colspan="3">
						<table width="100%">
							...
							<td  width="25%" align="left">
								...
								<hdiits:radio name="fromRad" id="fromRadId1" mandatory="true"  validation="sel.isradio" value="I"   errCaption="From"  default="${CorrVO.cmnLookupMstByCorrType.lookupName}"  captionid="WF.Internal" bundle="${fmsLables}" onclick="changelocDiv()"></hdiits:radio>
								<hdiits:radio name="fromRad"  id="fromRadId2"  validation="sel.isradio" value="E" captionid="WF.External" bundle="${fmsLables}" onclick="ajaxfunction()" default="${CorrVO.cmnLookupMstByCorrType.lookupName}"></hdiits:radio>
							</td>
							<td  width="25%" align="left">
								 <div id="div_ddCorrFrom" style="display:none">
									<hdiits:select name="corrFrm" id="dropdown_corrFrm" sort="false" mandatory="true" validation="sel.isrequired" captionid="WF.CorrFrom" bundle="${fmsLables}" onchange="corrFrm_onchange()">
									<hdiits:option value="0" selected="true"><fmt:message key="WF.Select" bundle="${fmsLables}"></fmt:message></hdiits:option>
										<c:if  test="${CorrVO.cmnLookupMstByCorrType.lookupName eq 'E'}">
													<c:forEach var="CorrespondenceTypeList" items="${CorrespondenceTypeList}">
														<c:if test="${LookupVO.lookupName eq (CorrespondenceTypeList.lookupName) }">
															<option value='<c:out value="${CorrespondenceTypeList.lookupId}"/>'  selected="true" name="${CorrespondenceTypeList.lookupShortName}">
															<c:out value="${CorrespondenceTypeList.lookupDesc}"/></option>
														</c:if>
														<c:if test="${LookupVO.lookupName ne (CorrespondenceTypeList.lookupName) }" >
															<option value='<c:out value="${CorrespondenceTypeList.lookupId}"/>' name="${CorrespondenceTypeList.lookupShortName}">
															<c:out value="${CorrespondenceTypeList.lookupDesc}"/></option>
														</c:if>
										 			</c:forEach>	
										</c:if>
			...
			<fmt:message key="WF.CorrAttach" bundle="${fmsLables}" var="attachmentTitle"></fmt:message>
				<jsp:include page="/WEB-INF/jsp/common/attachmentPage.jsp">
		                ...
		                <jsp:param name="formName" value="EditcorrespondenceForm" />
		       ...
		       <br>
		      		<center>
			      		<c:choose>
							...
							<c:otherwise>
					      		<c:if test="${CorrVO.wfOrgPostMpgMstByCrtPost.postId eq resValue.LoginPost}">
					      			<hdiits:button name="Corrsubmit_Update" value="Update" type="button" onclick="validatecontrols('update')"/>
					      		</c:if>
					      			<hdiits:button name="Corrsubmit_Can" value="Cancel" type="button" onclick="closeForm()"/>
...
<script language="javascript">
	...
	function validatecontrols(buttonParam)
	{
	   ...
	   if(document.getElementById('fromRadId2').checked)
	   	...
	   	if(document.getElementById('DDTextDecisionFlag_Id').value=="DD")
	   	{
		...
		if(returnVal == true)
		{
			...
			if(buttonParam != 'update')
				document.forms[0].action='${contextPath}/hdiits.htm?actionFlag=insertCorrespondence&buttonId='+buttonId;
			else
				document.forms[0].action='${contextPath}/hdiits.htm?actionFlag=editCorrespondence';
	...
	function backtoHomePage()
	{
		if(confirm("Do you really want to exit?"))
		{
			...
			document.forms[0].action = "${contextPath}/hdiits.htm?actionFlag=getHomePage";
			document.forms[0].method = "post";
			document.forms[0].submit();
		}
		else
	...
	/*function validate()
	{
		...
		alert("validateForm_CorrespondenceForm"+validateForm_CorrespondenceForm());
		var validData = validateForm_CorrespondenceForm();
		alert(validData);
		if( validData==true )
		{
...
/*	function fetchBranchSubject()
			...
			{    // Internet Explorer    
		        ...
		        xmlHttp.onreadystatechange = function()
				{
					...
					{     
						...
						{
							var XMLDoc=xmlHttp.responseXML.documentElement;
							var tableentries = XMLDoc.getElementsByTagName('SujectsMapped');	
							var comboid = document.getElementById('dd_brSub')
							if(comboid.length > 1)
						    		  {
						    		     clearList(comboid);
						    		  }
	...
	function fetchBranchSubject()
{
	clearList(document.EditcorrespondenceForm.brSub);
	clearList(document.EditcorrespondenceForm.section);

	clearList(document.EditcorrespondenceForm.crrCat);
	clearList(document.EditcorrespondenceForm.moreCat);
...
<c:if test="${CorrVO.cmnLookupMstByCorrType.lookupName eq 'I'}" >
	<script type="text/javascript">
		document.getElementById('locSearch_id').style.display = '';
		document.getElementById('div_ddCorrFrom').style.display = 'none';
		document.getElementById('corrAdd_id').style.display = 'none';
