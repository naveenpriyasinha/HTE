...
<body>
		...
		<%String displayButtons=(String)request.getParameter("displayButtons");%>
		<%String ifReadOnly=(String)request.getParameter("ifReadOnly");%>
		var richtext='<%=richText%>'
		var displayButtons = '<%=displayButtons%>'
		var ifReadOnly = '<%=ifReadOnly%>'
		if(displayButtons == 'N' && ifReadOnly == 'Y')
		{	
			writeRichText(richtext,'', 300, 230, false, true);		
		}
		else if(ifReadOnly == 'Y')
		{
			writeRichText(richtext,'', 300, 150, true, true);	
		}
		else if(displayButtons == 'N')
		{
			writeRichText(richtext,'', 300, 230, false, false);		
		}
