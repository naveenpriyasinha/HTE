...
var flagForUpdatedVO = false;
	...
	function addDataInTable(tableId, hiddenField, displayFieldArray, editMethodName, deleteMethodName, viewMethodName,checkMethod) 
	{
		...
		trow.insertCell(1).innerHTML ="<INPUT type='checkbox' name='FirstCheck" + counter + "' id='FirstCheck" + counter + "' value='YesChecked' onclick=showchecked('" + trow.id + "','" + counter + "')>";
...
/*function showchecked(rowids,cntr)
{
	...
	else
	{
		...
		//alert(abcd);
		alert(getrow.length);
		var depart=getrow.split(",");
		alert(depart.length);
		var newarr=new Array(depart.length);
...
function GetDesrd()
{	
	alert("inside getdesrd");
	alert(secndrow);
	var getdesired=secndrow;
	var bcde=getdesired.split(",");
	alert("length of getdesired="+bcde.length);
	for(var j=0;j<bcde.length;j++)
	{
		var getting=bcde[j];
		alert(getting);
		if(getting=='0')
		{
			alert("inside if"+getting);
		}
	}
}*/	
...
//		alert ("time"+time);
	 ...
	 *	
		...
		function populateFormForAllReadyAddedRecord()
	{
		if ((xmlHttp.readyState == 4) && (xmlHttp.status == 200))
		{
			...
			var temp1 = "cmnPersonMst/cmnContactMsts[0]/residencePhone";
			var finalValue = getValueFromDOM(xmlDOM, temp1);
			var temp2 = "srNo";
			var serialNo = getValueFromDOM(xmlDOM, temp2);
			alert ("serialNo :"+serialNo);
		}
	}
	 
	 
	 	function editMethodForAddedVO(rowId) 
	{
		alert('function editMethodForAddedVO called...')
		alert ('ROW ID  :'+rowId);
		sendAjaxRequestForEdit(rowId, 'populateFormForAllReadyAddedRecord');
		globalFlagDeleteDisable = true;	
	} 
	  
	 function isReferenceAttrPresent(xmlDOM,nodeStr)
		{
		alert("function isReferenceAttrPresent(xmlDOM,finalNodeStr) called.");
		var indexOfSqrBr = nodeStr.indexOf ("[");
		showAlert ("indexOfSetObject :"+indexOfSetObject);
		
		var finalNodeStr = nodeStr.substring (0,indexOfOpnSqr);



		var node = xmlDOM.selectNodes(nodeStr);
		var nodeObjPro = node[0].childNodes[0].getAttribute('reference');
		alert("nodeObjPro :"+nodeObjPro);
		if(nodeObjPro != null)
		{
			return true;
	...
	function getValueFromDOMForAllReadyAddedVO(domObj, propertyName) 
	{
		alert('getValueFromDOMForAllReadyAddedVO called...');
		alert(domObj);
		alert('doctype : ' + domObj.doctype);
		var entries = domObj.getElementsByTagName('cmnLookupMst');
		alert ('ENTRIES:'+entries);
		var entries1 = domObj.getElementByTagName('cmnLookupMst');		
		alert ('ENTRIES1:'+entries1);		
		var propName;
		for (var i = 0; i < entries.length; i++)
		{    			
			propName = entries[i].getAttribute('property');
	...
	function getValueFromDOMForPropertyArray(domObj, propertyNameArray ) 
	{
		//alert('getValueFromDOMForAllReadyAddedVO called...');
		//alert ('propertyNameArray.length :'+propertyNameArray.length)
		try 
		{
			var entries ;
			var lengthOfArray = propertyNameArray.length ;
			var finalNode ;
			var value = null;
			entries = domObj.childNodes;
			for(var i = 0;i<lengthOfArray;i++ )
			{
//				alert('i :'+i);
//				alert('propertyNameArray[i]'+propertyNameArray[i]);
//				alert(entries.length )
				for(var j = 0;j<entries.length ; j++)
				{
//					alert('j :'+j);
//					alert('entries[j].nodeName :'+entries[j].nodeName);
					if(entries[j].nodeName == propertyNameArray[i])
					{
//						alert('222');
						if(i == (lengthOfArray - 1))
						{
//							alert('kkkkk');
//							alert (entries[j].nodeName);
							value  = entries[j].childNodes[0].nodeValue ;
//							alert ("value :"+ value);
						}
						else
						{
//							alert('*****');
							entries = entries[j].childNodes;
						}
						break;
					}
				}
...
//					alert('break main loop');
		...
		{
			alert ('Exception thrown.In.getValueFromDOMForAllReadyAddedVO...');
			return null;
	...
	function getValueFromDomForPropertyFromNewXML(domObj, propertyName) 
	{
		var entries = domObj.getElementsByTagName(propertyName);
		try
		{
			return entries[0].childNodes[0].text;
		}
		catch(e)
		{
			alert('EXCEPTION THROWN IN METHOD getValueFromDomForPropertyFromNewXML');
			return null;
		}	
	}
	function getValueFromDOM(xmlDOM, propertyName)
	{
		showAlert ("xmlDOM.nodeName :"+xmlDOM.nodeName);
		var nodeStr = "/"+xmlDOM.nodeName+"/"+propertyName;
		var indexOfOpnSqr = nodeStr.indexOf("[");
		var indexOfClsSqr = nodeStr.indexOf("]");
		showAlert ("indexOfOpnSqr :"+indexOfOpnSqr);
		showAlert ("indexOfClsSqr :"+indexOfClsSqr);
		if(indexOfOpnSqr < 0)
		{
			var returnValueNode = xmlDOM.selectNodes(nodeStr);
			var returnValue ;
			try
			{
				returnValue		= returnValueNode[0].childNodes[0].nodeValue;
				return returnValue;
			}
			catch(e)
			{
	//			alert ("ERROR CODE 1 : THROWN WHILE GETTING FROM SIMPLE NODE VALUE ");
				return null;
			}
		}
		
		var indexOfSetObject = nodeStr.substring (indexOfOpnSqr+1,indexOfClsSqr);
		showAlert ("indexOfSetObject :"+indexOfSetObject);
		
		var finalNodeStr = nodeStr.substring (0,indexOfOpnSqr);
		showAlert("finalNodeStr	:"+finalNodeStr)
		
		if(isSetTagPresent(xmlDOM,finalNodeStr))
		{
			finalNodeStr = finalNodeStr +"/set";
		}
		
		var node = xmlDOM.selectNodes(finalNodeStr);
		var childNodeName = node[0].childNodes;
		// there is no object in set like "<set/>"
		if(childNodeName.length == 0)
		{
			return null;
		...
		var setObjName = node[0].childNodes[0].nodeName;
		
		showAlert("SET OBJECT NAME :"+setObjName);

		finalNodeStr = finalNodeStr +"/"+setObjName +"["+indexOfSetObject+"]" ;
		showAlert("finalNodeStr  :"+finalNodeStr );

		var remain = nodeStr.substring (indexOfClsSqr+1,nodeStr.length);
		showAlert("REMAIN :"+remain);
		
		finalNodeStr = finalNodeStr + remain ;
		showAlert("finalNodeStr :"+finalNodeStr);
		
		var nodeTemp = xmlDOM.selectNodes(finalNodeStr);
		var finalNodeValue ;
		try 
		{
			finalNodeValue = nodeTemp[0].childNodes[0].nodeValue;
