	...
	public Collection findReportData(ReportVO report, Object criteria) throws ReportException {
		...
		try {
			...
			if (report.getReportCode().equals("700088")) {
				...
				SBQuery.append("MstEmp EM where CY.dcpsId = EM.dcpsId and CY.curTreasuryCD = :treasuryId and CY.curDdoCD = :ddoCode");
				...
				if (lEmpList != null && !lEmpList.isEmpty()) {
					for (Integer lInt = 0; lInt < lEmpList.size(); lInt++) {
						...
						Long empId = (Long) lArrObj[1];
						Date DOB = (Date) lArrObj[3];
						Double basicPay = (Double) lArrObj[5];
						...
						rowList.add("<b>Name of the Employee : " + empName + "</b>");
						dataList.add(rowList);

						rowList = new ArrayList();
						rowList.add(new StyledData("<b>Pension Account Number : " + dcpsId + "</b>", RightAlignVO));
						dataList.add(rowList);
						...
						rowList = new ArrayList();
						rowList
								.add("<b>From (Month) : " + dateFormat3.format(startDate).toString() + space(15) + "To (Month) : " + dateFormat3.format(endDate).toString()
										+ "</b>");
						dataList.add(rowList);
						...
						if (rs.next()) {
							...
							for (int i = 4; i <= 12; i++) {
								SBQuery = new StringBuilder();
								SBQuery.append("SELECT contribution FROM trn_dcps_contribution WHERE dcps_emp_id = " + empId + " AND FIN_YEAR_ID = " + yearId + " AND month_id = "
										+ i);
							...
							td.add("");
							td.add("");
							td.add("<b>" + new Float(Double.parseDouble(rs2.getString("int_contrb_employee"))) + "</b>");
