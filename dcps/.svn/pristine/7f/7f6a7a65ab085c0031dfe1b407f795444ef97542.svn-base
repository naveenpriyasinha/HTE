...
{
	if(document.forms[0].cmbSearch.value==2 || document.forms[0].cmbSearch.value==5)
		{
		...
		if(document.forms[0].cmbSearch2.value==2 || document.forms[0].cmbSearch2.value==5)
		{
		...
		{
			if(document.forms[0].cmbSearch.value==9)
			{
			...
			if(document.forms[0].cmbSearch2.value==9 && document.getElementById('id_txtSearch2').value !='')
			{
			...
			if(cmbSearch2.value != 0)
				...
				disableAllButtons();	   
	           showProgressbar();
			document.forms[0].action ='ifms.htm?actionFlag=searchVoucher&searchby='+search+'&searchby2='+search2+'&locCode='+document.getElementById("id_cmbSubTrsy").value;
			document.forms[0].submit();
	...
	<table align="center" width="95%">
			<tr>
				...
				Location : 
                         ...
                         <td align="right"> 
                        <select name="cmbSubTrsy"  id="id_cmbSubTrsy" style="width:300px" >
                            
                            <option value="${resValue.locCode}" }"> All </option>
		                            <c:forEach var="subList" items="${resValue.locList}" >
		                            <c:choose>
		                            <c:when test='${subList.id == resValue.prevLocCode}'>
		                             <option value='<c:out value="${subList.id}"/>' selected>
		                                <c:out  value="${subList.desc}"/>
		                              </option>
		                            </c:when>
		                            <c:otherwise>
		                             <option value='<c:out value="${subList.id}"/>'>
		                                <c:out  value="${subList.desc}"/>
		                              </option>
		                            </c:otherwise>
		                            </c:choose>
				...
				<td align="center" width="18%">
					  <hdiits:button name="btnSearch" id="id_btnSearch" type="button" value="Search [F12]" onclick="search(cmbSearch);" tabindex="13"/>
