...
function writeRTE(rte, html, width, height, buttons, readOnly) {
    ...
    document.writeln('		<td><img class="rteImage" src="' + imagesPath + 'hyperlink.gif" width="25" height="24" alt="insert link" title="link" onClick="FormatText(\'' + rte + '\', \'createlink\')"></td>');
   		//document.writeln('		<td><img class="rteImage" src="' + imagesPath + 'internet.gif" width="15" height="12" alt="insert web link" title="web link" onClick="showCorrespondence(\'' + rte + '\', \'link\')"></td>');
    //document.writeln('		<td><img class="rteImage" src="' + imagesPath + 'hyperlink.gif" width="15" height="12" alt="insert web link" title="web link" onClick="FormatText(\'' + rte + '\', \'link\')"></td>');
...
<%
   ...
   for(int i=1;i<count1;i++)
   {
    ...
    %>
        ...
        document.writeln('<option value="StdNotings" >[Standard Noting]</option>');	        
<%    
	List<String> displayNotingList=null;
	if(session.getAttribute("displayNotingList")!=null){
		displayNotingList=(List)session.getAttribute("displayNotingList");
		session.removeAttribute("displayNotingList");
	}else{
		ServiceLocator serv=null;
		displayNotingList=new ArrayList<String>();
		if(session.getAttribute("serviceLocator")!=null){
			serv=(ServiceLocator)session.getAttribute("serviceLocator");
		...
		List<CmnLookupMst> listCmnLookupMst=cmnLookupMstDAO.getAllChildrenByLookUpNameAndLang("FMS_Notings",Long.parseLong(langId));
		List<String> notingList=new ArrayList<String>();
		for (CmnLookupMst cmnLookupMst2 : listCmnLookupMst) {
			displayNotingList.add(cmnLookupMst2.getLookupDesc());
		}
	}
	
	
   for(int i=0;i<displayNotingList.size();i++)
   {
	   
	  
	%>  
        document.writeln('<option value="<%=displayNotingList.get(i)%>" ><%=displayNotingList.get(i)%></option>');	    
    <%
      }   
    %>
    document.writeln('</select>');
...
function showCorrespondence(rte, command, option)
{
		//var url="hdiits.htm?actionFlag=showaddedCorrespondenceWithFile&Criteria=corr&fileId=${fileId}&DocFilterKey=2&showOnlyClose=true&docId=${docId}&richtextId="+rte,'','status=no,toolbar=no,scrollbars=yes,width=1500,height=700';
		//window.open('hdiits.htm?actionFlag=getDocListOfWorkList&fileId=${fileId}&DocFilterKey=2&showOnlyClose=true&docId=${docId}&richtextId='+rte,'','status=no,toolbar=no,scrollbars=yes,width=1500,height=700');
		...
		if ((command == "forecolor") || (command == "hilitecolor")) {
			...
			if (document.getElementById('cp' + rte).style.visibility == "hidden") {
			 ...
			 //save current values
			      parent.command = command;
			      currentRTE = rte;
			      window.open("hdiits.htm?actionFlag=fms_showaddedCorrespondenceWithFile&Criteria=Correspondence&fileId=${fileId}&DocFilterKey=2&showOnlyClose=true&docId=${docId}&richtextId="+rte,'','status=no,toolbar=no,scrollbars=yes,width=800,height=350,top=150,left=150');
...
function insertHyper(html,curRTE) {
			curRTE.execCommand('insertHTML', false, html);
