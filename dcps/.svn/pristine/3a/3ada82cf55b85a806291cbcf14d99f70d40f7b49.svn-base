...
tinyMCE.init({
	...
	theme_advanced_buttons1_add : "fontselect,fontsizeselect,undo,redo,insertdate,inserttime,preview,forecolor,print,bullist,numlist,pagebreak",			
...
function prepareTemplate()
{
	...
	var url='hdiits.htm?actionFlag=FMS_prepareDraftTemplate&ComOrSubjectDraftFlag='+comOrSub+'&fileId=${fileId}&type='+type;
...
{	
	...
	var comOrSub=document.getElementById('ComOrSubjectDraftFlag').value;	
	var url='hdiits.htm?actionFlag=FMS_getAttachmentIdForDraft&ComOrSubjectDraftFlag='+comOrSub+'&fileId=${fileId}&draftName='+reply+'&draftApproveFlag='+document.getElementById("draftApproveFlag").value+'&attchmentId='+document.getElementById("attchmentId").value+'&tamplateId='+type+'&draftCreateFlag='+document.getElementById('draftCreateFlag').value;
...
{		
	...
	//htmlContent = '<?xml version="1.0" encoding="UTF-8"?> <!DOCTYPE stylesheet[<!ENTITY nbsp "&#160;">]><html><body>'+ htmlContent + '</body></html>';
	htmlContent = '<?xml version="1.0" encoding="UTF-8"?><!DOCTYPE  document  PUBLIC  "specialEntities.dtd" '+subPath+'><html><body>'+ htmlContent + '</body></html>';
	var encodedHtml = encodeBase64(htmlContent);
	var xmlHttp=GetXmlHttpObject();
	if (xmlHttp==null) 
	...
	//alert('<fmt:message key="WF.GetContent" bundle="${wfLables}"></fmt:message>'+loopCount);
	var url='hdiits.htm?actionFlag=FMS_viewDraftByAttachmentId&fileId=${fileId}&attchmentId='+srno;
	var param = "encodedContent="+encodedHtml;
	//alert(url);
	xmlHttp.onreadystatechange=function()
	{
...
function ajaxfunction(src){			
		...
		var url;	
		if(src=='common'){
			url = "${contextPath}/hdiits.htm?actionFlag=FMS_showDraft&templateType=common&fileId=${fileId}";
			document.getElementById('ComOrSubjectDraftFlag').value='common'
			}
		else{
			url = "${contextPath}/hdiits.htm?actionFlag=FMS_showDraft&templateType=subject&fileId=${fileId}";
	...
	if(document.getElementById('draftRefDocFlag').value==1){
		...
		for(var cnt=0;cnt<radLength && flag==true ;cnt++)
		{
			if(radioObj[cnt].checked == true)
			{
				...
				var action="${contextPath}/hdiits.htm?actionFlag=approveDraft";	
		...
		if(flag==true)
		{
			alert('please select atleast one draft to approve or this is approved draft');
			return;
		}
...
function submitForm()
{
	...
	var action='${contextPath}/hdiits.htm?actionFlag=fetchFileDraft&fileId=${fileId}&pkval=${fileId}';	
...
function chageRefDocTemplateDetails()
{
	...
	var action='${contextPath}/hdiits.htm?actionFlag=updateRefDocTemplateDetails&fromFileDraftFlag=yes&fileTabDtlsIds='+val;
	alert('action'+action);
	var ed = tinyMCE.get('editor1');
	ed.setContent('');
	document.getElementById('testArea').value='';
	...
	<fmt:message key="WF.PREPARETEMP" bundle="${wfLables}" var="prepare"></fmt:message>
		...
		<display:table list="${fmsFileTabDtls}" pagesize="12" requestURI="" id="row"  style="width:100%"  >
			...
			<c:if test="${row.status eq 'PENDING'}">
			<display:column  titleKey="WF.DraftName" headerClass="datatableheader"><a href="javascript:getDraftForAttachment(${row.refFileCorrId},${appCount},${row.srNo},'${row.status}','${row.refCtgryMpgId}','${row.metaDataExist}')" >${row.draftName} </a></display:column>
			<c:set var="appCount" value="${appCount+1}"></c:set>
			</c:if>
			
			<c:if test="${row.status eq 'APPROVED'}">
			<display:column  titleKey="WF.DraftName" headerClass="datatableheader"><a href="javascript:getDraftForAttachment(${row.refFileCorrId},${doNotIssueCount},${row.srNo},'${row.status}','${row.refCtgryMpgId}','${row.metaDataExist}')" >${row.draftName} </a></display:column>
...
<script>
		...
		{			
			...
			var action='${contextPath}/hdiits.htm?actionFlag=FMS-givemanualcontrol&attachmentId='+id+'&fileId=${fileId}';		
			//alert(action);
		...
		{			
			...
			if(index2==-1){
				var urlStyle ='width=1000,height=1000,toolbar=no,menubar=no,location=no'; 
				window.open('hdiits.htm?actionFlag=generatePdf&fileTabId='+id+'&fileId=${fileId}','test',urlStyle);
			}else{
		...
		{		
			...
			var url='hdiits.htm?actionFlag=getHistoryDocument&attachemntId='+document.getElementById('attchmentId').value+'&fileId='+fileId;
			window.open(url,'test',urlStyle);
		}
