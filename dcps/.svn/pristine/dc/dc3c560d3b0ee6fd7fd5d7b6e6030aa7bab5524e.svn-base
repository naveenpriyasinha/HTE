			...
			if (report.getReportCode().equals("700016")) {
				...
				if (rs.next()) {
					...
					for (int i = 4; i <= 12; i++) {
						...
						if (rs1.next()) {
							balanceEmp = balanceEmp
									+ Float.parseFloat(rs1
											.getString("contribution"));
							balanceEmplr = balanceEmplr
									+ Float.parseFloat(rs1
											.getString("contribution"));
					...
					td.add("");
					td.add("<b>"
							+ new Float(Double.parseDouble(rs2
	...
	public List getAllTreasuries(String lStrLangId, String lStrLocId) {
		...
		try {
			...
			lsb = new StringBuffer(
					"select CM.loc_Id , CM.loc_Name from Cmn_Location_Mst CM where department_Id = 100003 and CM.LANG_ID = 1 order by CM.loc_Name ");
