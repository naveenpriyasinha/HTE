...
function SearchMedicine(RowId,XmlFilePath)
	{
			...
			if(xmlHttp.readyState == 4) 
			{
				if (xmlHttp.readyState == 4) 
					{ 				  		
						...
						/*else
						{
							var XmlPathsCheckFrmDb=getXPathValueFromDOM(xmlDOM, 'MedicineXMlPath');
							alert(XmlPathsCheckFrmDb.length);
							if(XmlPathsCheckFrmDb.length>0)
							{
								var SplitPathsCheckFrmDb=XmlPathsCheckFrmDb.split(",");
								for(var m=0;m<SplitPathsCheckFrmDb.length;m++)
								{
									alert("paths============="+SplitPathsCheckFrmDb[m]);
									alert("APths Checks======"+SplitPathsCheckFrmDb[m].substring(SplitPathsCheckFrmDb[m].length-2,SplitPathsCheckFrmDb[m].length));
									if(SplitPathsCheckFrmDb[m].substring(SplitPathsCheckFrmDb[m].length-2,SplitPathsCheckFrmDb[m].length)=="_D")
									{
										alert("ab sahi hona chahiye ");
										BillDataComingFromDB=true; 
									}
								}
							}
		...
		var href='hdiits.htm?actionFlag=getMediSearchMainPage&RowId='+RowId+'&XmlFilePath='+XmlFilePath+'&BillDataComingFromDB='+BillDataComingFromDB;
...
<hdiits:fieldGroup titleCaptionId="MRB.patdtl" bundle="${cmnLables}" id="MRB.patdtlId" collapseOnLoad="false">	
	...
	<table width="100%">
		...
		<!-- <tr bgcolor="#386CB7">
				...
				<fmt:message key="MRB.DISEASE_DETAILS" bundle="${cmnLables}" /> </U></strong></font></td>
			</tr> -->	
			<tr>
	...
	<center>			<hdiits:button name="AddDesease" type="button" value="Add" captionid="MRB.Add" bundle="${cmnLables}"
					onclick="insRowDeseaseTab();" />
				<hdiits:button type="button" name="Savee" value="Save" captionid="MRB.Update" bundle="${cmnLables}"
					readonly="true" onclick="startupAjaxFormValidationDisease()"></hdiits:button>
	</center>
	...
	<table width="100%">
		...
		<!-- <tr bgcolor="#386CB7">
				<td colspan="4" class=fieldLabel align=middle><font color="#ffffff"><strong> <U>
					<hdiits:caption captionid="MRB.TRTMNT_DTLS" bundle="${cmnLables}" /></U></strong></font></td>
			</tr> -->
			<tr>
	...
	<br>
			<center>	<hdiits:button name="Addhsptl" type="button" value="Add" captionid="MRB.Add" bundle="${cmnLables}"
					onclick="insRowTrtmntTab()" />
				<hdiits:button type="button" name="Savehsptl" value="Save" captionid="MRB.Update" bundle="${cmnLables}"
					readonly="true" onclick="startupAjaxFormValidationHsptl()"></hdiits:button>
			</center>
	...
	<table width="100%">
		<TBODY>
			<!-- <tr bgcolor="#386CB7">
				<td colspan="4" class=fieldLabel align=middle><font color="#ffffff"><strong> <U>
					<hdiits:caption captionid="MRB.APPROVER_DTLS" bundle="${cmnLables}" /></U></strong></font></td>
			</tr> -->
			<br>
			<tr>
				<td width="25%"><hdiits:caption captionid="MRB.APPROVER_NAME" bundle="${cmnLables}"/> </td>
				<td width="25%"><hdiits:text name="ApproverName" id="ApproverName" mandatory="true" captionid="MRB.APPROVER_NAME" bundle="${cmnLables}" validation="txt.isrequired" /> </td>
				
				<td width="25%"><hdiits:caption captionid="MRB.APPROVER_DESIG" bundle="${cmnLables}"/> </td>
				<td width="25%"><hdiits:text name="ApproverDesig" id="ApproverDesig" mandatory="true" captionid="MRB.APPROVER_DESIG" bundle="${cmnLables}" validation="txt.isrequired" /> </td>
			</tr>
			<tr>
				<td width="25%"><hdiits:caption captionid="MRB.HSPTL_NAME" bundle="${cmnLables}"/> </td>
				<td width="25%"><hdiits:text name="ApproverHsptl" id="ApproverHsptl" mandatory="true" captionid="MRB.APPROVER_NAME" bundle="${cmnLables}" validation="txt.isrequired" /> </td>
	...
	<table width="100%">
		<TBODY>
			...
			<tr>
				...
				<td width="25%"><hdiits:text name="ChemistName" id="ChemistName" /> </td>
			</tr>
		</TBODY>
	</table>
	<br>
...
<div id="tcontent3" class="tabcontent" tabno="2" >
	...
	<table width="100%" align="center">
		...
		<!-- 	<tr bgcolor="#386CB7">
				<td class=fieldLabel align=middle><font color="#ffffff"><strong> <U>
					<hdiits:caption captionid="MRB.MISC_DTLS" bundle="${cmnLables}" /> </U></strong> </font></td>
			</tr> -->
		</table>
		<br>
	<table>
	...
	<script>
				...
				document.getElementById('formTable1MRBAttachment').firstChild.firstChild.style.display='';
	</script>
	...
	<hdiits:hidden name="ReqType" id="ReqType"/>
		...
		<c:set var="PenReq" value="PendingReq"></c:set>
		<c:set var="AppReq" value="ApproveReq"></c:set>
		<c:set var="DraftReq" value="DraftReq"></c:set>
		<c:set var="RejReq" value="RejectReq"></c:set>
		<c:set var="CanReq" value="CancelReq"></c:set>
		<c:if test="${ReqType==PenReq || ReqType==AppReq}">
			<table align="center" width="100%">
			<tr align="center">
				<td><hdiits:button 	type="button" name="Submit" id="Submit" value="Submit As New Request" onclick="SubmitAsNewRequestInExistREQ()" captionid="MRB.SubmitAsNew" bundle="${cmnLables}"></hdiits:button>
					<hdiits:button type="button" name="SaveDraft" id="SaveDraft" value="Save as New Draft" onclick="saveAsNewDraftInExistREQ()" captionid="MRB.SaveAsNew" bundle="${cmnLables}"/>
		...
		<c:if test="${ReqType==DraftReq }">
		<table align="center" width="100%">
			<tr align="center">
				<td><hdiits:button 	type="button" name="Submit" id="Submit" value="Submit" 	captionid="MRB.Submit" bundle="${cmnLables}" onclick="SubmitWithChangesInExistREQ()"></hdiits:button>
					<hdiits:button type="button" name="SaveDraft" id="SaveDraft" value="Save as New Draft" onclick="saveAsNewDraftInExistREQ()" style="display:none" captionid="MRB.SaveAsNew" bundle="${cmnLables}"/>
					<hdiits:button type="button" name="SaveAgn" id="SaveAgn" value="Save Again" onclick="SaveAgainWithSameIdInExReq()" captionid="MRB.SaveAgn" bundle="${cmnLables}"/>
					<hdiits:button name="Close" type="button" value="Close"  captionid="MRB.Close" bundle="${cmnLables}" onclick="history.go(-1);return false;"/></td>
			</tr>
		</table>
		</c:if>
		<c:if test="${ReqType==RejReq|| ReqType==CanReq }">
		<table align="center" width="100%">
			<tr align="center">
				<td>
					<hdiits:button name="Close" type="button" value="Close"  captionid="MRB.Close" bundle="${cmnLables}" onclick="history.go(-1);return false;"/>
				</td>
			</tr>
		</table>
		</c:if>
