...
<c:set var="mandatoryFlagAddress"></c:set>
		...
		<hdiits:fieldGroup titleCaption="${finalTitleAddress}" mandatory="${mandatoryFlagAddress}" id="fieldGroupAddress${name}" collapseOnLoad="${collapseAddress}">
			...
			<table width="100%" id="countryStateTable${name}" >
			      <tr>
			      		<td style="visibility: hidden;" width="25%"><hdiits:caption captionid="ADDRESS.Country" bundle="${commonLablesAddress}"/></td>
			      		<td width="25%">
			      			<hdiits:select style="visibility: hidden;" sort="false" name="cmbCountry${name}" id="cmbCountry${name}" captionid="ADDRESS.Country" bundle="${commonLablesAddress}" validation="sel.isrequired"    onchange="populateState('cmbCountry${name}','cmbState${name}','getAllStates','${pageContext.request.contextPath}','countryCode','${name}')">
			      			...
			      			<label style="visibility: hidden;" class="mandatoryindicator">*</label>	
			      		</td>
