...
<!-- Added By Shivram -->
<c:set var="sevenpcBasic" value ="${resValue.sevenpcBasic}"></c:set> 
<!-- Ended By Shivram -->
...
<c:set var="revenueStamp" value="${resValue.revenueStamp}" ></c:set>
...
function checkBrokenPeriod(){
	var url="ifms.htm?actionFlag=checkBrokenPeriod&Month=${Month}&Year=${Year}&billNo=${billNo}";
...
function generateBrokenPeriod(myAjax)
{
	var XMLDoc = myAjax.responseXML.documentElement;
	var XmlHiddenValues = XMLDoc.getElementsByTagName('XMLDOC');
	var isGenerated= XmlHiddenValues[0].childNodes[0].firstChild.nodeValue;
	if (isGenerated=="no"){
		alert('Broken Period is not present for any employee.');		
	} 
	else{
		var url="ifms.htm?actionFlag=BrokenPeriodReport&Month=${Month}&Year=${Year}&billNo=${billNo}";
		...
		var urlstyle = "height=" + height + ",width=" + width + ",toolbar=no,minimize=no,status=yes,menubar=no,location=no,scrollbars=yes,top=0,left=0";
 		window.open(url,'_blank', urlstyle);
	}
}
...
var reportCode=value;
    ...
    /* Added By Shivram 25092020 */
    else if(value==5000099)
    {
    	//alert('Inside Revenue Stamp');
    	urlstring = "hrms.htm?actionFlag=reportService&reportCode=5000099&action=generateReport&FromParaPage=TRUE&customReportPage=ReportingFrameworkReportPagePopup&backBtn=0&Month=${Month}&Year=${Year}&Bill No=${billNo}&$FLAG=${billNo} ";
    }
	/* Ended By Shivram 25092020 */
...
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
			...
			<br>
			&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; 
			<a	href="javascript:void(0)" onclick="checkBrokenPeriod()"> 
				<font size="3">Broken Period</font> 
			</a>
...
<c:if test="${row.gis gt 0 or row.centralGis gt 0 or row.gisIps gt 0 or row.gisIas gt 0 or row.gisIfs gt 0  or row.gisZp gt 0}">
...
<c:if test="${row.HRR gt 0 or row.incomeTax gt 0 or row.professionTax gt 0 or row.pli gt 0 or row.accPolicy gt 0 }"> <!-- or row.revenueStamp gt 0 -->
