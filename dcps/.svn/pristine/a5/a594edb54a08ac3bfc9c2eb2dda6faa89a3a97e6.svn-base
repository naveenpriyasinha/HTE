...
//	alert ("searchEmpXPath :"+searchEmpXPath.length );
	...
	*/
	...
	function GoToNewPage<%=request.getParameter("SearchEmployee")%>() 
	{
    	...
    	var srchTextNm= document.getElementById('txtSearchName');
    	//var srchTextNm= document.getElementsByTagName('Employee_srchNameText_<%=request.getParameter("SearchEmployee")%>');

		var srchText;
    	if(document.getElementById('txtSearchName'))
    	{
    	
    	srchText= srchTextNm.value;  
    //	alert(srchText );
    	}
...
//    	alert("hidnSpecLoc 234234"+hidnSpecLoc);
		...
		else
		{
	        ...
	        var url = requestURL+ "?actionFlag=payrollSearchEmployeeByAll&SearchEmployee=<%=request.getParameter("SearchEmployee")%>&hidden_OwnLocationParam=<%=request.getParameter("ownlocation")%>&hidden_functionName="+functionName+"&hidden_levelSortParam="+levelSortParam+"&hidden_childLocSrchParam="+childLocSearch+"&hidden_multiSelStatus="+multiEmplSelect+"&hidden_hidnSpecLocId="+hidnSpecLoc+"&hidden_srchNameText="+srchText+"&hidden_SearchFlag=true"+"&ajaxCallFlag=true"+"&hidden_empExpDateSrchFlag"+empExpDateSrchFlag+"&hiddenSpecDsgnCodes="+hidnSpecDsgnCodes+"&hiddenSpecDepttCodes="+hidnSpecDepttCodes;
		        ...
		        //alert(url);
			xmlHttpFirstLevel.open("POST", encodeURI(url) , true);
...
//				    	alert(entries);
						for ( var i = 0 ; i < entries.length ; i++ )
	     				{
	     					var empSrchName = entries[i].childNodes[0].firstChild.nodeValue;   
	     					   
	     				    var srchCount = entries[i].childNodes[1].firstChild.nodeValue;
							...
							document.getElementById(srchCntHidnCtrl).value = srchCount;
	     				    if(srchCount == 'One')
	     				    {
	     				    	...
	     				    	document.getElementById(txtEmpIdCtrl).value	= entries[i].childNodes[2].firstChild.nodeValue;
	     				    	document.getElementById(txtEmpnameCtrl).value	= entries[i].childNodes[2].firstChild.nodeValue;
	     				    	document.getElementById(txtEmpLocCtrl).value	= entries[i].childNodes[4].firstChild.nodeValue;
	     				    	document.getElementById(txtEmpDesigCtrl).value	= entries[i].childNodes[6].firstChild.nodeValue;
	     				    	document.getElementById(txtEmpUserNameCtrl).value	= entries[i].childNodes[7].firstChild.nodeValue;
	     				    	document.getElementById(txtEmpUserIdCtrl).value	= entries[i].childNodes[8].firstChild.nodeValue;
	     				    	//alert(entries[i].childNodes[10].text);
	     				    	document.getElementById(txtEmpSevIdCtrl).value	= entries[i].childNodes[10].firstChild.nodeValue;
	     				    	var funcCallName = entries[i].childNodes[9].firstChild.nodeValue;
...
<tr>
<td>
Search With Name/HTESevaarthId</td>
...
<tr>  
   ...
   <c:choose>    
	    ...
	    <c:otherwise>  
    	   	<td class="fieldLabel" width="25%">HTESevaarth Id</td>	 
   			 <c:choose>
				 <c:when test="${empty empSearchVO}">
	   				 <td><hdiits:text  name="Emp_Sevarth_Id_${name}"  readonly="true"  mandatory="false" validation="txt.isrequired"  captionid="EMP"  bundle="${lables}" ></hdiits:text></td>		
   				 </c:when>
  ...
  <tr>
 	...
 	<c:choose>
    	...
    	<c:otherwise>  
		     ...
		     <hdiits:text default="${empSearchVO.locDistrictName}" name="DISTRICT_NM_${name}"  readonly="true"  captionid="DISTRICT_NM"  bundle="${lables}" ></hdiits:text></td>		
	     </c:otherwise>
    </c:choose>
