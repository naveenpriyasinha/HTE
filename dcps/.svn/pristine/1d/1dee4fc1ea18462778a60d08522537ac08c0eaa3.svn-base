...
ECOTree = function (obj, elm, url, groupId) {
	this.config = {
		iMaxDepth : 100,
		iLevelSeparation : 40,
		...
		expandedImage : 'themes/hdiits/images/workflow_treeImg/less.gif',
		collapsedImage : 'themes/hdiits/images/workflow_treeImg/plus.gif',
		transImage : 'themes/hdiits/images/workflow_treeImg/trans.gif'
	}
...
ECOTree.prototype._drawTree = function () {
	...
	var color = "";
	var border = "";
	// search the furthest node placed left of the border
	var xShift = 0;
	for (var n = 0; n < this.nDatabaseNodes.length; n++){
		node = this.nDatabaseNodes[n];
		xShift = (node.XPosition < xShift) ? node.XPosition : xShift;
	}
	xShift = Math.abs(xShift);
	if (xShift != 0){ // move all nodes to the right
		for (var n = 0; n < this.nDatabaseNodes.length; n++){
			node = this.nDatabaseNodes[n];
			node.XPosition += xShift;
		} 
	}
			...
			switch (this.render)
			{
				case "CANVAS":
					...
					if (node.canCollapse) {
						...
						s.push('<img border=0 width=20% height=35% src="'+((node.isCollapsed) ? this.config.collapsedImage : this.config.expandedImage)+'" >');							
						s.push('</a>');
						s.push('<img width=20% height=35% src="'+this.config.transImage+'" >');						
					}					
...
//					alert(this.config.selectMode + " : " + ECOTree.SL_NONE);

					if(node.id == document.getElementById("departmentId_SelTreeNode").value)
					{
						node.isSelected = true;
						document.getElementById("departmentId_SelTreeNode").value = departmentId_SelTreeNode;
					}
					
					if (this.config.selectMode != ECOTree.SL_NONE)
					{
						//s.push('href="javascript:'+this.obj+'.selectNode(\''+node.id+'\', true);" ');										
						//var link ="href='#' onClick=javascript:window.open('jstlTrial.jsp?id="+node.id+"')";
...
//					alert(" 1 " + node.dsc);
					...
					if (node.target && this.config.useTarget)
					{
						...
						var dept_fileId=document.getElementById('dept_fileId').value;
						var dept_corrId=document.getElementById('dept_corrId').value;
						
						
						var methodFlag=document.getElementById('methodFlag').value;
						
						//s.push('<a href="'+node.target+'">');
						var link ="<a onmouseover=style.cursor='pointer' href='hdiits.htm?viewName=wf-locationBranchSrch&deptId="+node.id+"&methodFlag="+methodFlag+"&SendAsCorr="+SendAsCorr+"&fileId="+dept_fileId+"&corrId="+dept_corrId+"&urlupd="+this.url+"&groupId="+this.groupId+"'>";
					
						//alert("linkkkkkkk"+link);
						s.push(link);	
						s.push(node.dsc);			
						s.push('</a>');	
