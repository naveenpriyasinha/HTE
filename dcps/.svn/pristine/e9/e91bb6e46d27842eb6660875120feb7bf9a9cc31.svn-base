...
function search(cmbSearch)
{
		...
		{
			if(document.forms[0].cmbSearch.value==6)
			{
			...
			document.forms[0].action ='ifms.htm?actionFlag=searchSubTrsyVoucher&searchby='+search;
			document.forms[0].submit();
		}
}
...
<hdiits:form name="frmSearch" validate="true" method="post">
	...
	<table align="center" width="90%">
			<tr>
				...
				<td width="18%">
					<hdiits:text name="txtSearch" id="id_txtSearch" size="15" default="${param.txtSearch}" tabindex="2" maxlength="36"/>
					<script>
				...
				<td width="20%">
					<hdiits:button name="btnSearch" type="button" value="Search [F12]" onclick="search(cmbSearch)" tabindex="13"/>
