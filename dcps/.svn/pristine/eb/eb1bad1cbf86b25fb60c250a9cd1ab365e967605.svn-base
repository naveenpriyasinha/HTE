		...
		<th width="100%" align="center">
			...
			<c:if test="${empty lngRequestId=='false'}" >
				<c:if test="${statusApproveReject=='Approved'}" >
					<fmt:message bundle="${FVISITLables}" key="FVISIT.FvisitReportApprovalMsg"/> <c:out value="${lngRequestId}"/> is <fmt:message bundle="${NOCLables}" key="NOC.Approve"/>
				</c:if>
				<c:if test="${statusApproveReject=='Rejected'}" >
					<fmt:message bundle="${FVISITLables}" key="FVISIT.FvisitReportApprovalMsg"/> <c:out value="${lngRequestId}"/> is <fmt:message bundle="${NOCLables}" key="NOC.Reject"/>
				</c:if>	
				<c:if test="${statusApproveReject=='Forwarded'}" >
					<fmt:message bundle="${FVISITLables}" key="FVISIT.FvisitReportApprovalMsg"/> <c:out value="${lngRequestId}"/> is <fmt:message bundle="${NOCLables}" key="NOC.Forward"/>
				</c:if>				
			</c:if>
			<c:if test="${empty lngRequestId=='true'}" >
				<fmt:message bundle="${FVISITLables}" key="FVISIT.SubmitFvisitReportMsg"/><c:out value="${reqId}"/> to <c:out value="${fwdTo}"/>
			</c:if>
			</center>
		</th>
	</tr>
