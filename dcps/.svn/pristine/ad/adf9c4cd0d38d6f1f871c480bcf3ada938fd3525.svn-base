...
<hdiits:form name="frm1" validate="true" method="POST"
	...
	<center>
		<font color='RED' ><c:out value="${cancleMessage}"></c:out></font>
	</center>
...
<hdiits:fieldGroup titleCaptionId="HRMS.Trip_Details" bundle="${caption}" expandable="false"/>
	...
	<center >
		...
		<tr>
			...
			<td>
					<c:out value="${tourDtlVO.tourName}"></c:out>
		...
		<tr>		
			...
			<td>
					<c:out value="${tourDtlVO.purpose}"></c:out>
...
<hdiits:fieldGroup titleCaptionId="HRMS.Journey_Details" bundle="${caption}" expandable="false"/>
	...
	<!-- Main Table -->
	<table id='detailsTable' border="1"  cellpadding="1" cellspacing="1" >
		<!-- Start: Heading -->
		 ...
		 <tr height="25%" class="odd">
			...
			<td rowspan="1" colspan="2" align="left">
				<hdiits:select	name="depart_plc${journeyRow.srNo}" size="1" caption="departure_place" tabindex="3" readonly="${journeyRow.viewFlag}"
					validation="sel.isrequired" id="depart_plc${journeyRow.srNo}"  default="${journeyRow.departCity}"
					onchange="">
			...
			<td rowspan="1" colspan="2" align="left">
				<hdiits:select readonly="${journeyRow.viewFlag}"
					...
					validation="sel.isrequired"  id="arr_plc${journeyRow.srNo}" 
			...
			<hdiits:select readonly="${journeyRow.viewFlag}" 	name="con_mod${journeyRow.srNo}" default="${journeyRow.convyanceMode}" id="con_mod${journeyRow.srNo}" caption="con_mode0" 
			...
			<td rowspan="2" colspan="1" nowrap="nowrap" height="25%" width="5%" style="text-align: center">
			<hdiits:select style="display:none"  name="accom${journeyRow.srNo}" readonly="${journeyRow.viewFlag}" classcss="mandatorycontrol" id="accom${journeyRow.srNo}" caption="accomodation${journeyRow.srNo}"   mandatory="false"
		...
		<table id="advanceDtl" style="">
			...
			<tr>
				...
				<td>
					<c:out value="${travelMstVO.permissibleAmt}"></c:out> 
				</td>
			</tr>
			<tr>
				<td>
					<a href="#" onclick="showTADACalculation(this)">show Details </a>
				</td>
			</tr>
			<tr>
				<td>
			...
			<tr>
				...
				<td>
					<hdiits:select name="modOfPay" id="modOfPay"  default="${travelMstVO.cmnLookupMst.lookupId}" >
						<hdiits:option  value="321611"><fmt:message key="HRMS.Select" bundle="${caption}" /></hdiits:option>
