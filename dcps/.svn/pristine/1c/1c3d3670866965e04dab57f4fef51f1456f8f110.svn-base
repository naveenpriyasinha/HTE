...
import com.tcs.sgv.nps.dao.FinYearWiseDCPSandGPFEmpCountAndExpDAOImpl;

public class FinYearWiseDCPSandGPFEmpCountAndExpServiceImpl extends DefaultReportDataFinder implements ReportDataFinder {
	

	
	private static final Logger gLogger = Logger.getLogger("PensionpayReports");
	private FinYearWiseDCPSandGPFEmpCountAndExpDAOImpl gObjRptQueryDAO = null;
	...
	public Collection findReportData(ReportVO lObjReport, Object criteria) throws ReportException {
		...
		try {
			...
			gObjRptQueryDAO = new FinYearWiseDCPSandGPFEmpCountAndExpDAOImpl(null, gObjSessionFactory);
			...
			if (lObjReport.getReportCode().equals("80000950")) {
				lLstDataList = getFinancialYearWiseDCPSandGPFEmpCountAndExpReport(lObjReport, gStrLocCode,ddocode);
			}
	...
	private List getFinancialYearWiseDCPSandGPFEmpCountAndExpReport(ReportVO lObjReport, String lStrLocCode,
			String ddocode) {
		System.out.println("Hiiii i am here");	
		List lArrReturn = new ArrayList();
		
		FinYearWiseDCPSandGPFEmpCountAndExpDAOImpl gObjRptQueryDAO = new FinYearWiseDCPSandGPFEmpCountAndExpDAOImpl(null, gObjSessionFactory);
		...
		try {
			...
			lArrReturn=gObjRptQueryDAO.getFinancialYearWiseDCPSandGPFEmpCountAndExpReport(lStrLocCode, ddocode);
		}
