...
<fmt:setBundle basename="resources.eis.eisLables" var="nomineeLables" scope="request"/>
<html>
<head>
<c:set var="contextPath" scope="request">${pageContext.request.contextPath}</c:set>	
<script type="text/javascript" src="script/common/address.js"></script>
<script type="text/javascript" src="<c:url value="/script/hrms/eis/addRecord.js"/>"></script>
...
<script type="text/javascript">		
		...
		function View(reqId)
		{			
			document.frmApproveNominee.action ="hrms.htm?actionFlag=ShowNomineeDtlsForApprove&reqId="+reqId;
			document.frmApproveNominee.submit();
		}		
		function increment()
		{
			v=v+1;
...
<body>
<hdiits:form name="frmApproveNominee" validate="true" method="POST" encType="multipart/form-data">
	<div id="tabmenu">
		<ul id="maintab" class="shadetabs">
			<li class="selected"><a href="#" rel="tcontent1"><b><hdiits:caption captionid="eis.nominee_dtls" bundle="${nomineeLables}"></hdiits:caption></b></a></li>
		</ul>
	</div>
	...
	<!-- 	<table class="tabtable" align="center">	
		...
		<table class="tabtable" border="1">			
			...
			<tr>
				...
				<td>
					<a href=javascript:void('view') onclick=javascript:View('${reqId}')>View</a>
				</td>					
			</tr>
			</c:forEach>				
		</table>
		...
		<c:choose>
			<c:when test="${flag != f }">
			<center><b><hdiits:caption captionid="eis.approvemsg" bundle="${nomineeLables}"></hdiits:caption><c:out value="${showreqId}"></c:out></b></center>
			</c:when>
			<c:otherwise>
				<center><b><hdiits:caption captionid="eis.request" bundle="${nomineeLables}"></hdiits:caption>  <c:out value="${showreqId}"></c:out> <hdiits:caption captionid="eis.succ_fwd" bundle="${nomineeLables}"></hdiits:caption></b></center>
			</c:otherwise>
		</c:choose>-->
	</div>
	</div>
