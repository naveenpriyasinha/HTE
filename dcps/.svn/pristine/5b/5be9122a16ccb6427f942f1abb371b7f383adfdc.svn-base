...
<fmt:setBundle basename="resources.hr.roster.search.search" var="comLable"
	scope="request" />
...
<hdiits:form name="searchEmployee" id="searchEmployee" validate="true"
	...
	<c:set var="i" value="0" /> <% int a=0; %> 
		...
		<display:column class="tablecelltext" title=" Select"
			...
			<hdiits:radio id="check${i}" name="check" value="${row.empId}~${row.empfName} ${row.empmName} ${row.emplName}~${row.userId}~ ${row.userName}~${row.postId}~${row.postName}~${row.dsgnId}~${row.dsgnName}~${row.locId}~${row.locName}~${row.depId}~${row.depName}"  onclick="checkclick(this);" />
		</display:column>
