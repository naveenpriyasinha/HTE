...
<c:set var="hrAllowTypeMst" value="${resValue.hrAllowTypeMst}" > </c:set>
...
{ 
  ...
  xmlHttp=GetXmlHttpObject();
		  ...
		  var url; 
		  var uri='';
		  url= uri+'&oldname=${actionList.ruleDescription}&ruleDesc='+ document.insExprFrm.txtRuleDesc.value;
...
<hdiits:form name="insExprFrm" validate="true" method="POST" 
	         action="hrms.htm?actionFlag=insertExprData&edit=Y" encType="multipart/form-data">

<div id="tabmenu">
	<ul id="maintab" class="shadetabs">
		<li class="selected"><a href="#" rel="tcontent1"><b><hdiits:caption captionid="EX.updateRule" bundle="${commonLables}"/></b></a></li>
	</ul>
	</div>
...
<TR> 
			...
			<TD> <hdiits:text readonly="true" style="BACKGROUND:GRAY" 
			     maxlength="10" default ="${actionList.ruleId}" 
				name="txtRuleID"
				 caption="Rule ID" /></TD>	
	</TR>
 ...
 <TR>
			...
			<TD colspan=2> <hdiits:text
				name="txtRuleDesc" mandatory="true" default="${actionList.ruleDescription}"
...
<tR>
<td>
<hdiits:hidden   name="allowance"   default="${hrAllowTypeMst.allowCode}"/> </td>
</tR>
 <TR>
			<TD class="fieldLabel" >
			<b><hdiits:caption captionid="EX.AT" bundle="${commonLables}"></hdiits:caption></b>
			</TD>
				<td><hdiits:select sort="false" captionid="EX.AT" bundle="${commonLables}" name="allowancecmb" readonly="true" mandatory="true" validation="sel.isrequired"> 
						<hdiits:option value="Select">-----------Select-----------</hdiits:option>
						
		<c:forEach var="allow" items="${allowNames}">
			<c:choose>
			<c:when test="${allow.allowCode == hrAllowTypeMst.allowCode}">
			<% System.out.println("If Loop"); %>
 			    <hdiits:option  value="${allow.allowCode}" selected="true"> ${allow.allowName} </hdiits:option>		
			  </c:when>
			  <c:otherwise>
 			    <hdiits:option  value="${allow.allowCode}"> ${allow.allowName} </hdiits:option>
			</c:otherwise>
			</c:choose>
			</c:forEach>										    										
		</hdiits:select>
...
<tR></tR>
 ...
 <TR>
			...
			<hdiits:textarea rows="3" cols="50" name="txtExp"  default="${actionList.ruleExpression}"
...
<tr>
			...
			<hdiits:text
				name="txtVal"
				 captionid="EX.VAL" default="${actionList.ruleValue}" bundle="${commonLables}" maxlength="10" onfocus="chkValue()"/>
			</TD>
		...
		<TD><b><hdiits:dateTime captionid="EX.STDATE"  bundle="${commonLables}" default="${actionList.ruleEffStartDate}" name="txtRuleSDate" validation="txt.isrequired,txt.isdt" /></b></TD>	
		<td><b><hdiits:caption captionid="EX.ENDATE"  bundle="${commonLables}"/></b></td>
		<TD><b><hdiits:dateTime captionid="EX.ENDATE" bundle="${commonLables}" name="txtRuleEDate" default="${actionList.ruleEffEndDate}"  validation="txt.isrequired,txt.isdt"/></b></TD>	
        </tr>
        <tr>
