...
import com.tcs.sgv.nps.dao.FinYearWiseNoOfEmpRetiredDAOmpl;

public class FinYearWiseNoOfEmpRetiredServiceImpl extends DefaultReportDataFinder implements ReportDataFinder {
	

	

	private static final Logger gLogger = Logger.getLogger("PensionpayReports");
	private FinYearWiseNoOfEmpRetiredDAOmpl gObjRptQueryDAO = null;
	...
	public Collection findReportData(ReportVO lObjReport, Object criteria) throws ReportException {
		...
		try {
			...
			gObjRptQueryDAO = new FinYearWiseNoOfEmpRetiredDAOmpl(null, gObjSessionFactory);
			...
			if (lObjReport.getReportCode().equals("80000952")) {
				lLstDataList = getNoOfEmpRetiredReport(lObjReport, gStrLocCode,ddocode);
			}
	...
	private List getNoOfEmpRetiredReport(ReportVO lObjReport, String lStrLocCode, String ddocode) {
		
		System.out.println("Hiiii i am here");	
		List lArrReturn = new ArrayList();
		
		FinYearWiseNoOfEmpRetiredDAOmpl gObjRptQueryDAO = new FinYearWiseNoOfEmpRetiredDAOmpl(null, gObjSessionFactory);
		...
		try {
			...
			lArrReturn=gObjRptQueryDAO.getNoOfEmpRetiredReport(lStrLocCode, ddocode);
		}
