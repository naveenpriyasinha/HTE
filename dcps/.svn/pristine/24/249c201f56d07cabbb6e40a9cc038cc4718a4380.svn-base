...
<c:set var="fmsCorrTabDtls" value="${resultMap.fmsFileTabDtls}"></c:set>
...
<%@ taglib uri="http://displaytag.sf.net" prefix="display" %>
	...
	<c:set var="appChkCount" value="0"> </c:set>
		<display:table list="${fmsCorrTabDtls}" pagesize="12" requestURI="" id="row"  style="width:100%"  >
...
function createDraft()
{
	...
	{
		alert('please select at least one type of category');
		return;
	}		
	...
	//alert('draft Id is'+draftId);
		...
		action="${contextPath}/hdiits.htm?actionFlag=InserttabdetailForCorrDraft";
		...
		if(document.getElementById('draftRefDocFlag').value==1){
			...
			for(var cnt=0;cnt<radLength && flag==true ;cnt++)
			{
				if(radioObj[cnt].checked == true)
				{
					...
					action="${contextPath}/hdiits.htm?actionFlag=approveDraftForCorr";
					document.getElementById("filedraftForm").method="post";
					document.getElementById("filedraftForm").action=action;	
					alert(action);
					document.getElementById("filedraftForm").submit();
...
function update(){
		...
		for(var cnt=0;cnt<radLength && flag==true ;cnt++)
		{
			if(radioObj[cnt].checked == true)
			{
				...
				action="${contextPath}/hdiits.htm?actionFlag=updateDraftForCorr";
				document.getElementById("filedraftForm").method="post";
				document.getElementById("filedraftForm").action=action;	
				document.getElementById("filedraftForm").submit();
...
function ajaxfunction(src){			
		...
		var url;	
		if(src=='common'){
			url = "${contextPath}/hdiits.htm?actionFlag=showTemplatesForCorr&templateType=common&corrId=${corrId}";
			document.getElementById('ComOrSubjectDraftFlag').value='common'
			}
		else{
			url = "${contextPath}/hdiits.htm?actionFlag=showTemplatesForCorr&templateType=subject&corrId=${corrId}";
