...
<hdiits:form name="frm1" validate="true" method="POST" encType="multipart/form-data" >
	...
	<center><input type="radio" name="newreq" value="newreq"
		onclick="changePage(this)" checked="checked"> New Travel
	Request <input type="radio" name="newreq" value="drafts"
		onclick="changePage(this)"> Previous Request</center>
	...
	<ul id="maintab" class="shadetabs">
		<li class="selected"><a href="#" rel="tcontent1" name="tcontent1" >
	...
	<table id="mainTable">
		<tr>
			...
			<td><hdiits:text mandatory="true" name="ref_file_no"
	...
	<table bgcolor="#386CB7" align="center" width="100%" id="journeyLable">
	<tr align="center">
		<td><font class="Label3" align="center"><u><font
	...
	<table id='detailsTable' border="1" cellpadding="0" cellspacing="0">
		...
		<thead title="Heading">
			...
			<tr class="datatableheader">
				<td colspan="1" width="10%"><fmt:message key="HRMS.Place"
					bundle="${caption}" /></td>
				<td colspan="1" width="5%"><fmt:message
					key="HRMS.Departure_Date" bundle="${caption}" /></td>
				<td colspan="1" width="1%"><fmt:message key="HRMS.Time"
					bundle="${caption}" /></td>
				<td colspan="1" width="10%"><fmt:message key="HRMS.Place"
					bundle="${caption}" /></td>
				<td colspan="1" width="5%"><fmt:message key="HRMS.Arrival_Date"
					bundle="${caption}" /></td>
				<td colspan="1" width="1%"><fmt:message key="HRMS.Time"
					bundle="${caption}" /></td>
			</tr>
		...
		<tbody>
			<tr height="25%">
				...
				<td rowspan="2" colspan="1" height="25%" width="25%" align="center"><hdiits:text
					...
					onblur="arrivalTimeCheck(this)" /><label class='mandatoryindicator'>*</label></td>
				<td rowspan="2" colspan="1" height="25%" width="25%" align="center">
				...
				<td rowspan="2" colspan="1" height="25%" width="25%" align="center"><hdiits:text
					...
					onblur="departureTimeCheck(this)" mandatory="false" maxlength="5" /><label class='mandatoryindicator'>*</label></td>
				...
				</hdiits:select><label class='mandatoryindicator'>*</label></td>
				<td rowspan="2" colspan="1" height="25%" width="5%"><hdiits:select
				...
				<td rowspan="2" colspan="1" height="25%" width="2%"><hdiits:number
		...
		<tbody>
			<tr height="25%">
				...
				<td rowspan="2" colspan="1" height="25%" width="25%" align="center"><hdiits:text
					...
					mandatory="false" maxlength="5" /><label class='mandatoryindicator'>*</label></td>
				<td rowspan="2" colspan="1" height="25%" width="25%" align="center">
				...
				<td rowspan="2" colspan="1" height="25%" width="25%" align="center"><hdiits:dateTime
					name="depart_dt1" captionid="HRMS.DummyDeprDt" bundle="${caption}"  maxvalue="31/12/2099 00:00:00" onblur="checkdate(this)"
					mandatory="true" /></td>
				<td rowspan="2" colspan="1" height="25%" width="25%" align="center"><hdiits:text
	...
	<center>
	<table id="btTable">
		<hdiits:button type="button" name="addButton" value="Add Journey"
			id="addButton" captionid="ADD_Journey" onclick="addrow()" />
		<hdiits:button type="button" name="savetrip" value="Save Trip"
			id="savetrip" captionid="Save_Trip" onclick="addtrip('updateRecord')"
			style="display:none" />
		<hdiits:button type="button" name="deleteButton"
			value="Delete Journey" id="deleterow" captionid="Delete_journey"
			onclick="deleteRows()" style="display: none" />
		<br>
		<br>
	</table>
	</center>
	...
	<table id="cntButton">
		<hdiits:button type="button" name="saveButton" value="Submit Request"
			id="SaveButton" captionid="save_trip" onclick="submitRequest()" />
		<hdiits:button type="button" name="saveasButton" value="Save as Draft"
			id="saveasButton" captionid="save_trip" onclick="saveasdraft()" />
		<hdiits:button type="button" name="addtripButton" value="Add Trip"
			id="addtripbt" captionid="add_trip" onclick="addtrip('addRecord')" />
		<hdiits:button type="button" name="closeButton" value="Close"
			id="closeButton" captionid="Close" onclick="closewindow()" />
...
<script type="text/javascript">
	...
	function finalSubmit(urlstyle)
	{
		...
		hid.value=row;
 		...
 		var agree=confirm("<fmt:message key="HRMS.wanttosubmitdetail" bundle="${alertLables}" />");
     	if (agree)
     	{
		...
		var SaveButton=document.getElementById('SaveButton');
	   	...
	   	saveasButton.style.display="";
			if(!generateDtlsArray())
			{
			  ...
			  addOrUpdateRecordforTravel(flag,'addTrip&reqtype=ajax&valpresent=true',totArray);
...
//		alert("Ina chkrow function");
   ...
   var staticError="";
	function changePage(form)
	{
        ...
        var journeyLable=document.getElementById('journeyLable');
       	var error=document.getElementById('errorId');		
		...
		{	
			...
			cntButton.style.display="";
			btTable.style.display="";
			detailTable.style.display="";
			attachtb.style.display="";
			tripdtls.style.display="";
			tabmenu.style.display="";
			journeyLable.style.display="";
			error.innerHTML=staticError;
		
			
		}
		else
		{
			staticError=error.innerHTML;
			error.innerHTML="";				
			cntButton.style.display="none";
			tabmenu.style.display="none";
			btTable.style.display="none";
			req_type.style.display="";
			mainTable.style.display="none";
			detailTable.style.display="none";
			attachtb.style.display="none";
			tripdtls.style.display="none";
			journeyLable.style.display="none";
	...
	var month;
		...
		var depart_day=departure_date.substring(0,2);
 		...
 		else
 		{
			...
			if(depart_tm.length==5 && depart_tm.charAt(2)!=':')
			{
