...
<script type="text/javascript"  src="script/dcps/tierContribution.js"></script>
...
<c:set var="TierType" value="${resValue.TierType}"></c:set>
...
<input type="hidden" name="hidTierType" id="hidTierType" value="${TierType}"></input>
<input type="hidden" name="counter" id="counter" value="${counter}"></input>
<fieldset class="tabstyle">
<c:if test="${TierType == 'type1'}"><legend id="tierHeader"><b>Tier - I Contribution </b></legend></c:if>
<c:if test="${TierType == 'type2'}"><legend id="tierHeader"><b>Tier - II Contribution </b></legend></c:if>
...
<c:if test="${TierType == 'type1' }"><legend id="tableHeader"><b>Tier - I Details</b></legend></c:if>
<c:if test="${TierType == 'type2' }"><legend id="tableHeader"><b>Tier - II Details</b></legend></c:if>
 ...
 <input type="hidden" name="hidGridSize" id="hidGridSize" value="1" /> 
	...
	<c:set var="hidGridSize" value="${hidGridSize}"></c:set>
	...
	<display:setProperty name="paging.banner.placement" value="none" />	
		<display:column  headerClass="datatableheader"  title="<input name='chkSelect' type='checkbox' onclick='checkUncheckAll(this,\"chkSelect\");'/>" >
			<input type="checkbox" name="chkbxArr" id="chkbxArr${tblTierICntrnbtn_rowNum - 1}" value="${tblTierICntrnbtn[0]}"/>
		</display:column>
		...
		<display:column headerClass="datatableheader"  sortable="true" titleKey="CMN.DELETE" >
			<img name="Image" id="Image" src="images/CalendarImages/DeleteIcon.gif"  onclick="RemoveTableRow(this,'tblTierICntrnbtn')"  />
			 <script>
								document.getElementById("hidGridSize").value=Number(document.getElementById("hidGridSize").value) + 1;
				</script>
		</display:column>
	</display:table>
	</c:when>
	...
	<c:set var="hidGridSize" value="${hidGridSize}"></c:set>
		<table id="tblTierICntrnbtn" align="left" width="98%" cellspacing="0"border="1">
			<tr class="datatableheader" style="width: 90px">
				<td width="5%" class="HLabel"><input name='chkbxArr' id='chkbxArr${hidGridSize}' type='checkbox' onclick='checkUncheckAll(this)'/></td>
...
<table width="40%" align="center">
		<tr>
			...
			<td width="40%" align="left">
			  ...
			  <hdiits:button name="btnForwardToDdo" classcss="bigbutton" type="button" captionid="BTN.FORWARD"	bundle="${dcpsLables}" onclick="ForwardTierToDDO();" /> </td>
		</tr>
...
<table width="40%" align="center">
		<tr>
			...
			<td width="40%" align="left">
			  <hdiits:button name="btnReject"	type="button" captionid="BTN.REJECT" bundle="${dcpsLables}" onclick="rejectTierData();" />
			  <hdiits:button name="btnApprove"  type="button" captionid="BTN.APPROVE"	bundle="${dcpsLables}" onclick="approveTierData();" /> </td>
		</tr>
	<tr>
	<td><fmt:message key="CMN.REMARKS" bundle="${dcpsLables}"></fmt:message></td> 
		<td><textarea id="txtRemarks" name="txtRemarks"  style="height: 20px" ></textarea></td></tr>
		</table>
</c:if>
