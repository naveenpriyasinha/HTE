...
<fmt:setBundle basename="resources.eis.eisLables" var="approveFamilyLabels" scope="request"/>
<html>
<head>
...
<script type="text/javascript">		
		...
		{			
			document.frmApproveFamily.action ="hrms.htm?actionFlag=ShowFamilyDtlsForApprove&reqId="+reqId+"&empId="+empId;
			document.frmApproveFamily.submit();
		}		
		function increment()
		{
			v=v+1;
...
<body>
<hdiits:form name="frmApproveFamily" validate="true" method="POST" encType="multipart/form-data">
<div id="tabmenu">
	<ul id="maintab" class="shadetabs">
		<li class="selected"><a href="#" rel="tcontent1"><b><hdiits:caption captionid="eis.FamilyDtls" bundle="${approveFamilyLabels}"></hdiits:caption></b></a></li>
	</ul>
</div>
<div id="family" name="family">
	...
	<c:choose>
		<c:when test="${flag != f }">
		<center><b><hdiits:caption captionid="eis.approvemsg" bundle="${approveFamilyLabels}"></hdiits:caption><c:out value="${showreqId}"></c:out></b></center>
		</c:when>
		<c:otherwise>
			<center><b><hdiits:caption captionid="eis.request" bundle="${approveFamilyLabels}"></hdiits:caption>  <c:out value="${showreqId}"></c:out> <hdiits:caption captionid="eis.succ_fwd" bundle="${approveFamilyLabels}"></hdiits:caption></b></center>
		</c:otherwise>
	</c:choose>	
