...
function ShoPrevRecrd()
{
if(ShowTableFlag==false)
{
	document.getElementById("MRBReqTabelAppr").style.display="";
	ShowTableFlag=true;
}
else
{
...
function DOAmountEqual(q)
{
	document.MRBFRM.approvedAmt1.value=q.value;
}

function GetReqType()
{
	document.MRBFRM.rejectActionTaken.value=parent.document.getElementById('reject').value;
	return true;
}
...
<c:set var="ExistingRecord" value="${resValue.ExistingRecord}"></c:set>

<c:set var="admblamt" value="${mrbReqVO.requestedAmt}"></c:set>


<hdiits:form name="MRBFRM" validate="true" method="POST"
	action="./hrms.htm?actionFlag=frwrdmrb" encType="multipart/form-data">
	<br>
	...
	<table width="30%" >
		<tr>
			<td>
				<hdiits:caption captionid="MRB.ShowPrevdtl"
				bundle="${cmnLables}" />
			</td>
			<td >
				<hdiits:button name="ShowPrev" type="button" value="V" onclick="ShoPrevRecrd()"/>
			</td>
		</tr>
	</table>
	<c:set var="blank" value="[]"></c:set> 
	<c:if test="${ExistingRecord!=blank}">
	<table id="MRBReqTabelAppr" width="100%" border="1" style="border-collapse: collapse;display:none" borderColor="BLACK">
					<tr>
						<td align="middle" class="datatableheader"><hdiits:caption captionid="mrb.patName" bundle="${cmnLables}"/></td>
						<td align="middle" class="datatableheader"><hdiits:caption captionid="MRB.ReqStatus" bundle="${cmnLables}"/></td>
						<td align="middle" class="datatableheader"><hdiits:caption captionid="MRB.ReqAmnt" bundle="${cmnLables}"/></td>
						<td align="middle" class="datatableheader"><hdiits:caption captionid="MRB.ApprvAmnt" bundle="${cmnLables}"/></td>
						<td align="middle" class="datatableheader"><hdiits:caption captionid="MRB.ApplyDate" bundle="${cmnLables}"/></td>
					</tr>
					
					<c:forEach var="ExistingRecord" items="${ExistingRecord}">
					<tr>
						<c:if test="${ExistingRecord.dependantName==Self}">
						<td><fmt:message key="mrb.self" bundle="${cmnLables}" />
						</td>
						</c:if>
						<c:if test="${ExistingRecord.dependantName!=Self}">
						<td><c:out value="${ExistingRecord.dependantName}"></c:out></td>
						</c:if>
						<td><c:out value="${ExistingRecord.requestStatus}"></c:out></td>
						<td><c:out value="${ExistingRecord.requestAmnt}"></c:out></td>
						<td><c:out value="${ExistingRecord.approveAmnt}"></c:out></td>
						<td><fmt:formatDate value="${ExistingRecord.requestApplyDate}" pattern="dd/MM/yy"/></td>
					</tr>
					</c:forEach>
	</table>
	</c:if>
	<c:if test="${ExistingRecord==blank}">
	<table align="center" id="MRBReqTabelAppr" width="100%" style="display:none">
		<tr align="center"><td><font color="red"><b><fmt:message key="MRB.NotFound" bundle="${cmnLables}"/></b></font></td></tr>
	</table>
	</c:if>
		</hdiits:fieldGroup>
	...
	<table width="100%" >
		...
		<c:if test="${relation==Self}">
			<table width="50%">
				<tr>
					<td width="10%"><hdiits:caption captionid="MRB.pattype"
						bundle="${cmnLables}" /></td>
...
<table width="100%" >
		...
		<tr>
			<td colspan="4"><display:table list="${resValue.DiseaseMstDtls}"
				...
				style="width:100%; bgcolor:#C9DFFF;border-collapse: collapse;borderColor:BLACK; " >
				<display:column class="tablecelltext"  titleKey="MRB.DISEASE_TYPE"
				  headerClass="datatableheader" style="text-align: center;bgcolor:#C9DFFF;">${row.parentDisDesc }
				</display:column>
				<display:column class="tablecelltext" titleKey="MRB.DISEASE_CAT"
					headerClass="datatableheader" style="text-align: center;bgcolor:#C9DFFF;">${row.diseaseDesc }
				</display:column>
				<display:column  class="tablecelltext" titleKey="MRB.REMARKS"
					 headerClass="datatableheader"
					style="text-align: center;bgcolor:#C9DFFF;">${row.diseaseRemarks }
				</display:column>
			</display:table></td>
		</tr>
		<tr>
...
<table width="100%" >
		...
		<tr>	
			<table id="BillDtlsTab"  border="1" width="100%" style="border-collapse: collapse" borderColor="BLACK">
			<TBODY>
				<tr>
			...
			<c:forEach var="BillDtlList" items="${resValue.BillDtlList}">
				<tr>
 					...
 					<td>
 						<table id="MediDtl" border="1" width="100%" style="border-collapse: collapse" borderColor="BLACK">
 							<tbody>
 								<tr>
								...
								<c:set var="condition" value="checked" />
 								...
 								<tr bgcolor="lightblue">
 									<td><input type="checkbox" name="MediId" id="MediId" value="${MediList.medicineAmnt}/${MediList.mediDtlSrNo}" onclick="DeductOrAddAmount(this)" ${condition}/></td>
...
<tr></tr>
<tr>
	<td ><font color="red" size="2"><b><fmt:message
	key="MRB.NOTE_2" bundle="${cmnLables}" /></b></font></td>
</tr>
</table>
</hdiits:fieldGroup>
<br>
...
<table width="50%">
	<tr>
		<td><hdiits:caption captionid="mrb.bilamt" bundle="${cmnLables}" /></td>
		<td><b><c:out value="${BillTotal}"></c:out></b></td>
	</tr>
	<tr>
		<td><c:set var="miscAmt" value="0"></c:set> 
	...
	<tr>			
		<td id="hi">
			<hdiits:caption captionid="mrb.miscamt" bundle="${cmnLables}" />
		</td>
		<td>
			<b><c:out value="${miscAmt}"></c:out></b>
		</td>
			<hdiits:hidden name="micsamttot" default="${miscAmt}" id="miscamtot" />
	</tr>
	<tr>
		<td>
			<hdiits:caption	captionid="MRB.totadamt" bundle="${cmnLables}" id="totadamt" />
		</td>
		<td>
			<b><c:out value="${admblamt}"></c:out></b>
		</td>
	</tr>	
	<tr>
		<td>
			...
			<hdiits:caption captionid="MRB.APRVD_AMT" bundle="${cmnLables}" /> 
		</td>
		<td>
			<hdiits:number name="approvedAmt1" default="${TotalApprovAmnt+miscAmt}" id="appamt1" readonly="true"/>
		</td>
	</tr>		
</table>			
</div>
...
<hdiits:fieldGroup titleCaptionId="MRB.MISC_DTLS" bundle="${cmnLables}" id="MRB.MISC_DTLSId" collapseOnLoad="false">	
	<table width="100%">
		...
		<tr>
			...
			<table width="50%">
				...
				<tr>
					<td id="hi"><hdiits:caption	captionid="mrb.miscamt" bundle="${cmnLables}" /></td>
					<td><b><c:out value="${miscAmt}"></c:out></b></td>
					<hdiits:hidden name="micsamttot" default="${miscAmt}" id="miscamtot" />
				</tr>
				<tr>
					<td><hdiits:caption captionid="MRB.totadamt" bundle="${cmnLables}" id="totadamt" /></td>
					<td><b><c:out value="${admblamt}"></c:out> </b></td>
				</tr>
				<tr>
					<td>
						<hdiits:hidden name="iniApprovedAmt" default="${TotalApprovAmnt}" id="iniaprvamt" /> 
					...
					<td>
						<hdiits:number name="approvedAmt" default="${TotalApprovAmnt+miscAmt}" id="appamt" onblur="DOAmountEqual(this)"/>
					</td>
				</tr>
			</table>
	</hdiits:fieldGroup>
	...
	<table id=status align="center">
		<tr>
			<td colspan="4"><font color="red"><hdiits:caption captionid="MRB.status" bundle="${cmnLables}" /></font></td>
		</tr>
	</table>
	<script>	
	status();
...
<hdiits:hidden name="rejectActionTaken" id="rejectActionTaken"/>
	<!-- For attachment : End-->
<hdiits:jsField jsFunction="GetReqType()" name="GetReqType"/>
<script>		
		mdcnfrwd();
</script>
</div>
