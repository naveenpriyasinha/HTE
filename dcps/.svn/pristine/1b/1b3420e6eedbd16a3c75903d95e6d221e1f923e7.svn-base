...
<script>
  ...
  {
  			startProcess();
  			document.searchEmployee.action="./hdiits.htm?actionFlag=getEmpSearchData&userId=-1&multiple=${bIsMultiple}&searchFieldName=${SearchFieldName}";
	...
	<div class="tabcontentstyle" style="height: 100%">
		<div id="tcontent1" class="tabcontent" tabno="0"><br>
			...
			<c:set var="i" value="0" /> <% int a=0; %>
				...
				<display:table list="${resultValue.Employee}" id="row" requestURI="" pagesize="15"  export="false" style="width:100%" offset="1" >
				
				<fmt:formatDate var="retirementDate" value="${row.dtEmpSrvcDate}" pattern="dd/MM/yyyy" />
				...
				<display:column class="tablecelltext" title=""
					...
					<c:if test="${empty bIsMultiple or bIsMultiple eq true}">
						<hdiits:checkbox id="check${i}" name="check" value="${row.empId}~${row.empfName} ${row.empmName} ${row.emplName}~${row.userId}~${row.userName}~${row.postId}~${row.postName}~${row.dsgnId}~${row.dsgnName}~${row.locId}~${row.locName}~${row.depId}~${row.depName}~${retirementDate}~${row.lngEmpSrvcFlag}~${doj}"  onclick="checkclick(this);" />
					</c:if>
					<c:if test="${bIsMultiple eq false}">
						<hdiits:radio id="check${i}" name="check" value="${row.empId}~${row.empfName} ${row.empmName} ${row.emplName}~${row.userId}~${row.userName}~${row.postId}~${row.postName}~${row.dsgnId}~${row.dsgnName}~${row.locId}~${row.locName}~${row.depId}~${row.depName}~${retirementDate}~${row.lngEmpSrvcFlag}~${doj}"  onclick="checkRadio(this);" />
					</c:if>
				...
				<display:column class="tablecelltext" titleKey="location"
					headerClass="datatableheader" style="text-align: center"
					sortable="true">
					${row.locName} 
				</display:column>
