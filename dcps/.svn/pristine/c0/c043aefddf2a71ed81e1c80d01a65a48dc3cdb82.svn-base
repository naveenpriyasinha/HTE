...
function chkUserName()
{
	...
	var uri = 'ifms.htm?actionFlag=chkUserNameForATO';
...
function getResponseChkUname(myAjax)
{
	...
	}else if(Emp_name == "location"){
		alert("User Name does not belong to this location");
		document.getElementById("txtUserName").value = "";
		document.getElementById("txtEmpName").value = "";
		document.getElementById("btnUpdate").disabled = true;
	}else{
...
function reSetPassword()
{
	...
	var uri = 'ifms.htm?actionFlag=setPasswordATO';
...
function getResponsePWD(myAjax)
{
	...
	if(PWD == "Bday"){
		...
		alert("Your Password has been reset to: ifms123");
	}
