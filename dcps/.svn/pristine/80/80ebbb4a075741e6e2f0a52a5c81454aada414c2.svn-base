...
<c:set var="selectedDDOCodeForLevelTwo" value="${resValue.selectedDDOCodeForLevelTwo}" ></c:set>
<c:set var="selectedLocCode" value="${resValue.selectedLocCode}" ></c:set>
...
function filterByDDOCode(selectedLocCode){
	//alert("hiii i m in filtering");
	var ddoCode= document.getElementById("selectedLevelTwoLocCode").value;
	//alert(ddoCode);
	var locCode= document.getElementById("selectedLocCode").value;
	//alert(locCode);

	var url;

		url="./hrms.htm?actionFlag=viewAllReports&selectedLocCode="+locCode+"&selectedDDOCodeOfLevel2DDO="+ddoCode;
		document.printAllReports.action= url;
		document.printAllReports.submit();
}
function filterByReptDDOCode(selectedLevelTwoLocCode){
	//alert("hiii i m in filterinxdfdfffdfdfddg");
	var locCode= document.getElementById("selectedLevelTwoLocCode").value;
	//alert(locCode);
	var url;

		url="./hrms.htm?actionFlag=viewAllReports&selectedDDOCodeOfLevel2DDO="+locCode;
		document.printAllReports.action= url;
		document.printAllReports.submit();
}
</script>
...
<hdiits:form name="printAllReports" validate="true" method="POST" action=" ">
	...
	<br/>
	 <table width="100%" border="1" bordercolor="black"  align="center" id="searchTable" rules="">
      <tr>
      <td  width="100%" align = "center" style="background-color:#F7E7D7 ;color: rgb(202,97,12);font-size: small;font-style: normal;font-weight : bold">REPORTING DDO</td>
      </tr>
      <tr>
      <td align = "center" width="100%">
      		<hdiits:select name="selectedLevelTwoLocCode"  id="selectedLevelTwoLocCode" size="1" caption="Level 2 DDO Code" sort="false" onchange= "filterByReptDDOCode(selectedLevelTwoLocCode)" mandatory="true"> 
				<hdiits:option value="" selected="true">---------------------------------------Select-------------------------------------</hdiits:option>
			<c:forEach items="${reptDDO}" var="reptDDO">
			
			
				<c:choose>
				<c:when test="${reptDDO[0]==selectedDDOCodeForLevelTwo}">
						<hdiits:option value="${reptDDO[0]}" selected="true"> ${reptDDO[1]}(${reptDDO[0]})</hdiits:option>
				</c:when>
					<c:otherwise>
						<hdiits:option value="${reptDDO[0]}"> ${reptDDO[1]}(${reptDDO[0]})</hdiits:option>
					</c:otherwise>
				</c:choose>	
			</c:forEach>
			</hdiits:select>		       
	 ...
	 <table width="100%" border="1" bordercolor="black"  align="center" id="searchTable" rules="">
      <tr>
      <td width="100%" align = "center" style="background-color:#F7E7D7 ;color: rgb(202,97,12);font-size: small;font-style: normal;font-weight : bold">ASSISTANT DDO</td>
      </tr>
      <tr>
      <td width="100%" align = "center">
      		<hdiits:select name="selectedLocCode"  id="selectedLocCode" size="1" caption="Level 1 DDO Code" sort="false" onchange= "filterByDDOCode(selectedLocCode)" mandatory="true"> 
				<hdiits:option value="" selected="true">---------------------------------------Select-------------------------------------</hdiits:option>
			<c:forEach items="${asstDDO}" var="asstDDO">
			
			
				<c:choose>
				<c:when test="${asstDDO[2]==selectedLocCode}">
						<hdiits:option value="${asstDDO[2]}" selected="true"> ${asstDDO[1]}(${asstDDO[0]})</hdiits:option>
				</c:when>
					<c:otherwise>
						<hdiits:option value="${asstDDO[2]}"> ${asstDDO[1]}(${asstDDO[0]})</hdiits:option>
					</c:otherwise>
				</c:choose>	
			</c:forEach>
			</hdiits:select>		       
	...
	<table width="45%" border="1" bordercolor="black"  align="center" id="searchTable" rules="">
	<tr>
