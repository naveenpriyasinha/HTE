...
<script>
	function getListOfEmp() {
		...
		if (validate()) {
			...
			var treasList = document.getElementById("treasuryList").value
					.trim();

			if (treasList != null) {
				document.frmNSDL.action = "ifms.htm?actionFlag=ContributionNewList&treasno="
						+ treasList;
...
<title>Contribution Report For NSDL</title>
</head>
<body>
			...
			<table border="1" bordercolor="black" align="center" id="searchTable">
				...
				<tr>
					<td colspan="4" align="center">
						<center>
							<input type="button" onclick="getListOfEmp();" class="buttontag"
								value="View Records" />
						</center>
					</td>
				</tr>
	...
	<c:if test="${check==true}">
		<c:choose>
			<c:when test="${lstEmp!= null  && flagg == false}">
				...
				<fieldset id="datalistframe" style="margin: 10px;">
					<legend>All Employee Details</legend>
					<display:table list="${lstEmp}" id="ais"
