  ...
  return false; 
    ...
    try{   
			...
			if(emparray != ""){
					...
					var url = "hrms.htm?actionFlag=generateApprovedListTrnsByEmp&emparray="+emparray+"&relDt="+relDt;  
           ...
           if (xmlHttp.readyState == 4){
				...
				if (xmlHttp.status == 200){
						...
						var xmlDOM = getDOMFromXML(decXML);
						 ...
						 var serialNo = xmlDOM.getElementsByTagName('serialNo');	
						var empUserId = xmlDOM.getElementsByTagName('userId');	
   						...
   						var empDateJ = xmlDOM.getElementsByTagName('datej');
   						 var empTranLoc = xmlDOM.getElementsByTagName('locName');
				    	...
				    	for ( var i = 0 ; i < empName.length ; i++ ){
				    	 		 ...
				    	 		 var xmlfilepath = xmlFilePath[i].childNodes[0].text;
				    	 			...
				    	 			var sno = serialNo[i].childNodes[0].text;
				    	 		    ...
				    	 		    var displayFieldA  = new Array(name,loc,datej,dater,emptranloc,emprelDt);

									addDBDataInTableForTrs('ApprovedWithRelDt','ReltrnsPunch',displayFieldA,xmlfilepath,'editRecord','delRecord','',sno);
...
//	var userid = trowvalue[1].innerHTML;
	...
	var serailno = trowvalue[8].innerHTML;
	...
	document.ReleaseApprovedTransfered.releaseDt.value = trowvalue[6].innerHTML;
...
function updateDataInTableforTransfer(hiddenField, displayFieldArray) 
	{
		...
		var field = document.getElementById(displayFieldArray[0]);
			trow.cells[6].innerHTML = field.value;			
	}
...
document.ReleaseApprovedTransfered.action="hrms.htm?actionFlag=sendOnRequestRelDate";
...
<hdiits:fieldGroup titleCaptionId="trn.rlsempl" bundle="${transferLables}" expandable="true" mandatory="true" > 	
<table width="100%" id="ApprovedWithoutRelDt" name="ApprovedWithoutRelDt" 	border="1"  style="display:none">
	<hdiits:hidden name="serialno" id="serialno"  default=""/>
		<tr class="datatableheader">
           <!--  <td width="5%"><hdiits:caption captionid="trn.serailno"
				bundle="${transferLables}"></hdiits:caption></td> --> 
			<td width="5%" style="display:none"><hdiits:caption
				captionid="trn.serailno" bundle="${transferLables}"></hdiits:caption></td>
			
			<td width="5%"><hdiits:caption
				captionid="trn.serailno" bundle="${transferLables}" /></td>
			<td width="20%"><hdiits:caption captionid="trn.empNm"
				bundle="${transferLables}"></hdiits:caption></td>
			<td width="20%"><hdiits:caption captionid="trn.loc"
				bundle="${transferLables}"></hdiits:caption></td>
			<td width="15%"><hdiits:caption captionid="trn.dtJoin"
				bundle="${transferLables}"></hdiits:caption></td>
			<td width="15%"><hdiits:caption captionid="trn.transferDate"
				bundle="${transferLables}"></hdiits:caption></td>
			<td width="20%"><hdiits:caption captionid="trn.locto"
				bundle="${transferLables}"></hdiits:caption></td>
	...
	<table id="tblrelbtn" style="display:none">
	  <tr>
	      <td><hdiits:caption
			captionid="trn.releaseDt" bundle="${transferLables}" />
			<hdiits:dateTime name="releaseDt"  mandatory="true" captionid="trn.releaseDt" bundle="${transferLables}" maxvalue="31/12/2099"/></td>
	      <td>
	         <hdiits:button name="releaseBtn" type="button" id="releaseBtn"  bundle="${transferLables}" captionid="trn.releaseBtn"  onclick="getReleaseEmpLst()"/>
	         <hdiits:button name="updateBtn" type="button" style="display:none" id="updateBtn"  bundle="${transferLables}" captionid="trn.update" onclick="updateRecord()"/>
	...
	<table width="100%" id="ApprovedWithRelDt" name="ApprovedWithRelDt" border="1" style="display:none">
		<tr class="datatableheader">
            <td width="5%" style="display:none"><hdiits:caption
				captionid="trn.serailno" bundle="${transferLables}"></hdiits:caption></td>
			...
			<td width="15%"><hdiits:caption captionid="trn.locto"
				bundle="${transferLables}"></hdiits:caption></td>
			<td width="15%"><hdiits:caption captionid="trn.releaseDt"
				bundle="${transferLables}"></hdiits:caption></td>	
	...
	<table width="100%" align="center"  id="tblSubmit" style="display:none">
		...
		<td ><hdiits:button name="submitTrnfer" type="button"
			...
			onclick="insertData();"></hdiits:button>
			</td>
	...
	<c:if test="${not empty TransferVO}">
		<c:forEach items="${TransferVO}" var="TransferVO" varStatus="x">
			...
			<fmt:formatDate var="doj" value="${TransferVO.datej}"
				pattern="dd/MM/yyyy" />
			<fmt:formatDate var="dor" value="${TransferVO.dateR}"
				pattern="dd/MM/yyyy" />
			...
			<script type="text/javascript">
					    					...
					    					trow.insertCell(j+5).innerHTML='${dor}';	   					
