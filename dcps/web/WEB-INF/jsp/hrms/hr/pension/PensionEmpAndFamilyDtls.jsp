...
<c:set var="flag" value="${resValue.flag}"></c:set>
	<hdiits:fieldGroup collapseOnLoad="false" id="emp_family_field1" titleCaptionId="Pension.EmpAppName" bundle="${pensionEmpFamilyLabel}">		
	<table class="tabtable">		
		...
		<c:if test="${flag ne 'Branch'}">
			<tr>
				<td class="fieldLabel" width="25%"><b><hdiits:caption captionid="Pension.AddComm" bundle="${pensionEmpFamilyLabel}"></hdiits:caption></b></td>
				<td class="fieldLabel" width="25%">
					<hdiits:select captionid="Pension.AddComm" bundle="${pensionEmpFamilyLabel}" id="addForComm" name="addForComm" mandatory="true" onchange="getAddressForComm(this);" validation="sel.isrequired"> 
						<hdiits:option value="Select">--<fmt:message key="Pension.Select" bundle="${pensionEmpFamilyLabel}" />--</hdiits:option>
						<c:forEach var="addOfComm" items="${pensionAddforComm}">
		   					<option value="<c:out value="${addOfComm.lookupName}"/>">
							<c:out value="${addOfComm.lookupDesc}"/></option>						
						</c:forEach>					
					</hdiits:select>
				</td>
				<td class="fieldLabel" width="25%"><b><hdiits:caption captionid="Pension.CommAdd" bundle="${pensionEmpFamilyLabel}"></hdiits:caption></b></td>
				<td class="fieldLabel" width="25%">
					<hdiits:textarea name="cmmAddTextArea" id="cmmAddTextArea" readonly="true"></hdiits:textarea>
				</td>
			</tr>
		</c:if>				
	...
	<tr>
		<td class="fieldLabel" width="0%"></td>
		<td class="fieldLabel" width="100%" colspan="2">
		<jsp:include page="../../../common/address.jsp">
			<jsp:param name="addrName" value="communicationAddress" />
	...
	<BR>
	<hdiits:fieldGroup collapseOnLoad="false" id="emp_family_field2" titleCaptionId="Pension.FamilyDtls" bundle="${pensionEmpFamilyLabel}">			
	<BR>
