...
<hdiits:form name="searchEmployee" id="searchEmployee" validate="true"
	...
	<table align="center">
		...
		<tr></tr>
		<tr style="display: none">
			<td><hdiits:caption captionid="post" bundle="${comLable}" /></td>
			<td><hdiits:select name="dsgnId" size="1" caption="dsgnId"
				mandatory="false" onchange="OnChange(this,'post')" tabindex="4"
				sort="false">
				<hdiits:option value="0">-----Select-----</hdiits:option>
		...
		<c:if test="${not empty resultValue.Employee}" >
		...
		<display:table list="${resultValue.Employee}" id="row" requestURI=""
			pagesize="10" export="false" style="width:100%" offset="1" >
			...
			<display:setProperty name="paging.banner.placement" value="bottom" />
				...
				<c:if test="${row.isEmpCategoryExist != 'N'}">
				...
				<hdiits:radio id="check${i}" name="check"
					value="${row.empId}~${row.empfName} ${row.empmName} ${row.emplName}~
							    		${row.userId}~ ${row.userName}~${row.postId}~${row.postName}~
							    		${row.dsgnId}~${row.dsgnName}~${row.locId}~${row.locName}~
							    		${row.depId}~${row.depName}~${row.empDateOJ}~${row.empCategory}~${row.empCategoryId}"
					onclick="checkclick(this);" />
					</display:column>
				</c:if>	
			

			<display:column class="tablecelltext" title=" Sr.No"
				headerClass="datatableheader" value="<%=a=a+1 %>"
	...
	<c:if test="${empty resultValue.Employee}" >
	<tr>
	<td>
	...
	<table align="center">
		...
		<tr align="center">
			<td align="center"><hdiits:button name="Submit" type="button"
				...
				name="Reset" type="button" value="Reset" tabindex="12" /><hdiits:button name="Close" type="button" value="Close" onclick="closeWindows()" /> </td>
		</tr>
	</table>
