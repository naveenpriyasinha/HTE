...
<script type="text/javascript">
	...
	function adddata()
	{
		...
		//	alert("No of Tours="+respArray.length);
			...
			//	alert("TourName And Path==="+respArray[i]);
				...
				Trip_Name.value=IdArray[0];
				if(IdArray[1]!="1")
				{
				//	alert("Attachment id is not null and it is :::"+IdArray[1]);
					var rowNum = addDBDataInTableAttachment('tripdtls','encXML',new Array(IdArray[0]),xmlPath[0],IdArray[1],'','','editRecordForDraft');
				}else
				{
	...
	//				var rowNum=addDBDataInTable('tripdtls','encXML',new Array(IdArray[0]),xmlPath[0],'editRecordForDraft','deleteTravel','');
					var rowNum = addDBDataInTableAttachment('tripdtls','encXML',new Array(IdArray[0]),xmlPath[0],'','','','editRecordForDraft');
				}
...
//		alert("n a save as draft ");
		...
		//		alert("UrlStyle of Draft="+urlstyle);
	...
	function checkForDraft()
	{
		...
		if (xmlHttp.readyState == 4) 
		 { 				  
				...
				errorStr=errorStr+tripName+"</font><font color='RED'>";
				var indiError="";
				var checkStr="";
			for(var j=0;j<len;j++)
			{
						
						indiError=indiError+"<Br>For Journey"+j+"::";					
					...
					//	alert('arr_plc'+arr_plc);
						if(arr_plc=="-1")
						{
							indiError=indiError+"<fmt:message key="HRMS.Arrival_Place" bundle="${caption}" />";
							//alert('Please select Arrival Place in Journey'+j);
							checkStr=checkStr+"a";
						}
						depart_plc=getXPathValueFromDOM(xmlDOM, parentNode+'/travelToPlace/cityId');
					//	alert('depart_plc'+depart_plc);
						if(depart_plc=="-1")
						{
							//alert('Please select Departure Place in Journey'+j);
							indiError=indiError+","+"<fmt:message key="HRMS.Departure_Place" bundle="${caption}" />";
							checkStr=checkStr+"a";
						}
						

						con_mod=getXPathValueFromDOM(xmlDOM, parentNode+'/cmnLookupMstByTravelMode/lookupId');
					//	alert('con_mod'+con_mod);
						if(con_mod=="-1")
						{
							//alert('Please select Convyance Mode Journey'+j);
							indiError=indiError+","+"<fmt:message key="HRMS.Conveyance_Mode" bundle="${caption}" />";
							checkStr=checkStr+"a";
						}else if(con_mod=="500002" ||con_mod=="500022")
						{
						
						}else if(con_mod=='500003'||con_mod=="500033")
						{
								
								sub_train=getXPathValueFromDOM(xmlDOM, parentNode+'/cmnLookupMstByTrainName/lookupId')
								if(sub_train=="-1")
								{
									//alert('Please select Train Name Journey'+j);
									indiError=indiError+","+"<fmt:message key="HRMS.Train_Name" bundle="${caption}" />";
									checkStr=checkStr+"a";
								}
								...
								if(sub_class=="-1")
								{
									//alert('Please select Class  Journey'+j);
									indiError=indiError+","+"<fmt:message key="HRMS.Purpose" bundle="${caption}" />";
									checkStr=checkStr+"a";
									
								}
								
						}else if (con_mod=='500004' ||con_mod=="500044")
						{
								
								sub_own=getXPathValueFromDOM(xmlDOM, parentNode+'/cmnLookupMstByGovtOrGovtpaid/lookupId')
								if(sub_own=="-1")
								{
									//alert('Please select Owned by Journey'+j);
									indiError=indiError+","+"<fmt:message key="HRMS.Owned" bundle="${caption}" />";
									checkStr=checkStr+"a";
								}
								if(sub_own=="400427"||sub_own=="400428"||sub_own=="400527"||sub_own=="400528")
								{
									sub_veh=getXPathValueFromDOM(xmlDOM, parentNode+'/cmnLookupMstByVehicleType/lookupId')
									
									if(sub_veh=="-1")
									{
										//alert('Please select Vehicle Type Journey'+j);
										indiError=indiError+","+"<fmt:message key="HRMS.Vehicle_Type" bundle="${caption}" />";
										checkStr=checkStr+"a";
									}
								}
								...
								if(cost==" ")
								{
									//alert('Pleaes Enter Cost ');
									indiError=indiError+","+"<fmt:message key="HRMS.Total_Cost" bundle="${caption}" />";
									checkStr=checkStr+"a";
								}
					...
					//	alert('purpose'+purpose);
						...
						{	
								if(j!=1)
								{
									//alert('Please select Purpose of stay of Journey'+j);
									indiError=indiError+","+"<fmt:message key="HRMS.Purose_Of_Stay" bundle="${caption}" />";
									checkStr=checkStr+"a";
								}
						}
				...
				//		alert("travelFromDate"+tmp_dt);		
						...
						{
							//alert('Please select arrival Data in a Trip'+tripName);
							indiError=indiError+","+"<fmt:message key="HRMS.Arrival_Date" bundle="${caption}" />";
							checkStr=checkStr+"a";
						...
						if(arrayDate[1]=="00:00")
						{
							...
							indiError=indiError+","+"<fmt:message key="HRMS.Time" bundle="${caption}" />";
							checkStr=checkStr+"a";
						}
					...
					//	alert("travelToDate"+tmp_dt);
						if(arrayDate[0]=="11/11/1111")
						{
							//alert('Please select Departure Data in a Trip '+tripName);
							indiError=indiError+","+"<fmt:message key="HRMS.Departure_Date" bundle="${caption}" />";
							checkStr=checkStr+"a";
						}
						if(arrayDate[1]=="00:00")
						{
						...
						if(accom=="-1")
						 {
						 	...
						 	indiError=indiError+","+"<fmt:message key="HRMS.Accomodation" bundle="${caption}" />";
						 	checkStr=checkStr+"a";
						 }
				}
		...
		for(var i=0;i<dtlsrow;i++)
		{
			...
			cnt=cnt*1+1;
				...
				var arr_plc0=document.getElementById("arr_plc0");
 				...
 				var name=arr_plc0.options[arr_plc0.selectedIndex].text +" "+tmp+" "+arr_plc1.options[arr_plc1.selectedIndex].text+" - "+arr_dt0.value;		
 				var trippurpose=document.getElementById("purpose");
				var refFile=document.getElementById("ref_file_no");
   	 	     	var tourDtlslist=new Array();
   	 	     	tourDtlslist[0]=name;
   	 	     	var tourId=document.getElementById('tourId');
   	 	     //	alert('Tour Id in generating array method'+tourId.value);
   	 	     	if(tourId.value!="")
   	 	     	{
   	 	    // 		alert("Tour id for xml is set to ::"+tourId.value );
   	 	     		tourDtlslist[1]=tourId.value;
   	 	     	}else
   	 	     	{
	   	 //	     	alert("Tour id for xml is set to ::"+tourCnt);
...
//				var xmlDOM = populateAttachment(decXML,'frm1');
	...
	function checkvalue(i)
	{
			...
			if(depart_plc.value=="-1")
			{
				alert("<fmt:message key="HRMS.arrivalplaceinjourney" bundle="${alertLables}" />");   
				return false;
			}
			if( depart_dt.value=="")
			{
			...
			if(con_mod.value!="-1")
			{
				...
				{
					if(sub_train.value=='-1')
					{
						alert("<fmt:message key="HRMS.selecttrainmode" bundle="${caption}" />");   
						return false;
					}
					if(sub_class.value=="-1")
					{
				...
				if(con_mod.value=="Bus")
				{
					...
					}else if(sub_own.selectedIndex=="4"||sub_own.selectedIndex=="5")
					{
						if(sub_veh.value=="-1")
						{
							alert("<fmt:message key="HRMS.seelectvehicletype" bundle="${alertLables}" />");   
							return false;
						}
					}
					if(dis.value=="") 
...
//				alert('arr_plc');
	...
	//			alert('depart_tm');
			...
			if(con_mod.value.length!=2)
			{
				flag=1;
	//			alert('con_mod');
				return flag;
			}
			if(sub_train.value.length!=2)
			{
				flag=1;
	//			alert('subtrain');
				return flag;
			}
			if(sub_class.value.length!=2)
			{
				flag=1;
	//			alert('subclass');
				return flag;
			}
			if( sub_own.value.length!=2)
			{
	...
	//				alert('air mod==='+airmod.value);
			...
			if(purpose.value.length!=2)
			{
					flag=1;
		//			alert('purpose');
					return flag;
			}
			if(accom.value.length!=2) 
			{
					flag=1;
...
//				alert('Remark');
	...
	function submitRequest()
	{
		...
		var len=tb.rows.length;
 		...
 		if(NullCnt!=0)
 		{
	 		//	alert('Value is present ');
				...
				if(test())
   	 	     	...
   	 	     	{
   	 	     		alert("<fmt:message key="HRMS.neccessaryfields" bundle="${alertLables}" />");    
   	 	     	}
 		}
 		else
 		{
 		//	alert('All rows are empty !!!!!! '); 			
 			...
 			{
 				alert("<fmt:message key="HRMS.onetouralongwithrequest" bundle="${alertLables}" />");    
 				return;
 			}
 		}
	...
	function departureTimeCheck(form)
	{
		...
		{
			alert("<fmt:message key="HRMS.arrivaltime" bundle="${alertLables}" />");    
			form.value="";
			return;
		}
		...
		if(depart_tm.value=="")
		{
			alert("<fmt:message key="HRMS.inserttime" bundle="${alertLables}" />");    
		...
		if(month==dmonth&&year==dyear&&day==dday)
		{
			...
			{
				alert("<fmt:message key="HRMS.departutetimegtrrival" bundle="${alertLables}" />");  
				form.value="";
				return;
	...
	function departurechkdate(form)
	{
		...
		{
			alert("<fmt:message key="HRMS.selctprevdepartureplace" bundle="${alertLables}" />");    
			form.value="";
			return;
		}
		...
		}else if(idyear==iayear)
		{
			...
			}else if(idmonth==iamonth)
			{
				if(idday<=iaday)
				{
					...
					alert("<fmt:message key="HRMS.arivaldategtdeparturedate" bundle="${alertLables}" />");    
					form.value="";
					return;		
				}	
			}else
...
/*		if(month>dmonth||year>dyear||day>dday)
	...
	function ckeckplace()
	{
		...
		if(arr_plc0.value==-1)
		{
			alert("<fmt:message key="HRMS.selectarrivalname" bundle="${alertLables}" />");    
			return false;
		}else if(arr_plc1.value==-1)
		{
			alert("<fmt:message key="HRMS.selectdeparturename" bundle="${alertLables}" />");    
			return false;
		}
...
//		
	...
	function addrow()
	{
		...
		cell2.colSpan="1";
		cell2.rowSpan="2";
		var str="<select name='arr_plc"+apnd+"' size='1' caption='drop_down' id='arr_plc"+apnd+"'  mandatory='true' onchange='setpredepart(this)'> <option value='-1'>-----Select -------</option>";
		...
		cell5.colSpan="1";
		cell5.rowSpan="2";
		str="<select name='depart_plc"+apnd+"' size='1' caption='drop_down'  id='depart_plc"+apnd+"'  mandatory='true' onchange='setArrival(this)'> <option value='-1'>-----Select -------</option>";
	  	for(var i=0;i<cityCmb.length-1;i++)
		{
			str+="<option value='"+cityCmbVal[i]+"'>"+cityCmb[i]+"</option>";
		...
		cell8.colSpan="1";
		cell8.rowSpan="2";
		str="<select name='con_mod"+apnd+"' size='1' caption='drop_down' id='con_mod"+apnd+"'  mandatory='true' onchange='hide(this)'> <option value='-1'>-----Select -------</option>";
	...
	//End hide columns
		...
		cell19.colSpan="1";
		cell19.rowSpan="2";
		str="<select name='purpose"+apnd+"' size='1' id='purpose"+apnd+"' caption='drop_down'   mandatory='true' onchange=''> <option value='-1'>-----Select -------</option>";
		...
		cell20.colSpan="1";
		cell20.rowSpan="2";
		str="<select name='accom"+apnd+"' size='1' caption='drop_down' id='accom"+apnd+"'  mandatory='true' onchange=''> <option value='-1'>--------Select -----------</option>";
		...
		str="<select name='sub_train"+apnd+"' id='sub_train"+apnd+"' size='1' caption='drop_down'   mandatory='true' onchange='' disabled='true'> <option value='-1'>-----Select -------</option>";
	    ...
	    str+="</select>";
    	str+="<select name='sub_own"+apnd+"' id='sub_own"+apnd+"' size='1' caption='drop_down' style='display:none'  mandatory='true' onchange='enabletype(this)'> <option value='-1'>-----Select -------</option>";
		...
		cell2.style.display="";
		str="<select name='sub_class"+apnd+"' id='sub_class"+apnd+"' size='1' caption='drop_down'   mandatory='true' onchange='' disabled='true'> <option value='-1'>-----Select -------</option>";
	    ...
	    str+="</select>";
		str+="<select name='sub_veh"+apnd+"' id='sub_veh"+apnd+"' size='1' caption='drop_down' style='display:none'  mandatory='true' onchange='' disabled='true'> <option value='-1'>-----Select -------</option>";
	...
	function deleteRows() {
		...
		if(lCnt<=0)
		{
		     alert("<fmt:message key="HRMS.selactonerow" bundle="${alertLables}" />");    
		      return;
		}
		if(lCnt>1)
		{
...
<hdiits:form name="frm1" validate="true" method="POST"
	...
	<center><input type="radio" name="newreq" value="newreq"
		onclick="changePage(this)" > New Travel
	Request <input type="radio" name="newreq" value="drafts"
		onclick="changePage(this)" checked="checked"> Previous Request</center>
	
	
	<center><hdiits:select name="req_type" size="1"
	...
	<table id='detailsTable' border="1" cellpadding="0" cellspacing="0"	style="display: none">
		...
		<tbody>
			<tr height="25%">
				...
				<td rowspan="2" colspan="1" height="25%" width="25%" align="center"><hdiits:dateTime
					name="arr_dt0"  captionid="HRMS.DummyDeprDt" bundle="${caption}" mandatory="true" maxvalue="31/12/2099 00:00:00" /></td>
				...
				<td rowspan="2" colspan="1" height="25%" width="25%" align="center"><hdiits:dateTime
					name="depart_dt0" captionid="HRMS.DummyDeprDt" bundle="${caption}" maxvalue="31/12/2099 00:00:00" onblur="checkdate(this)"
					mandatory="true" /></td>
				<td rowspan="2" colspan="1" height="25%" width="25%" align="center"><hdiits:text
				...
				<td rowspan="2" colspan="1" height="25%" align="center"
					...
					name="airmod0" id="airmod0" value="-1" title="Business"
					onclick="setAirmodeval(this)" /></td>
				<td rowspan="2" colspan="1" height="25%" align="center"
					style="display: none" id="eco0">Economic <hdiits:radio
					name="airmod0" id="airmod0" value="-1" title="Economic"
					onclick="setAirmodeval(this)" /></td>
		...
		<tbody>
			<tr height="25%">
				...
				<td rowspan="2" colspan="1" height="25%" width="25%" align="center"><hdiits:dateTime
					name="arr_dt1" captionid="HRMS.DummyDeprDt" bundle="${caption}" onblur="departurechkdate(this)" maxvalue="31/12/2099 00:00:00"
					mandatory="true" /></td>
				<td rowspan="2" colspan="1" height="25%" width="25%" align="center"><hdiits:text
				...
				<td rowspan="2" colspan="1" height="25%" width="25%" align="center"><hdiits:dateTime
					name="depart_dt1" captionid="HRMS.DummyDeprDt" bundle="${caption}" onblur="checkdate(this)" maxvalue="31/12/2099 00:00:00"
					mandatory="true" /></td>
				<td rowspan="2" colspan="1" height="25%" width="25%" align="center"><hdiits:text
				...
				<td rowspan="2" colspan="1" height="25%" align="center"
					...
					name="airmod1" id="airmod1" title="Business" value="-1"
					onclick="setAirmodeval(this)" /></td>
				<td rowspan="2" colspan="1" height="25%" align="center"
					style="display: none" id="eco1">Economic <hdiits:radio
					name="airmod1" id="airmod1" title="Economic" value="-1"
					onclick="setAirmodeval(this)" /></td>
				<td rowspan="1" colspan="1" height="25%" align="center"
	...
	<hdiits:button type="button" name="saveButton"
		...
		onclick="submitRequest()" style="display:none" /> <hdiits:button
	...
	<center><font color="RED"><c:out value="No Record Found!!!!!" ></font> </c:out></center>
	</c:if>	
	<br>
	...
	<br>
	<br>
	<center><hdiits:button type="button" name="closeButton"
...
<c:forEach var="flagTypeVar" items="${flagType}">
	<script>
		var flagtype="${flagTypeVar}";
