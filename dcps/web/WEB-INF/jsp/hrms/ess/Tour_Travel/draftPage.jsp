...
<script type="text/javascript">
	...
	function adddata()
	{
		...
		//	alert("No of Tours="+respArray.length);
			...
			//	alert("TourName And Path==="+respArray[i]);
				...
				Trip_Name.value=tmpArray[0];

				if(IdArray[1]!="1")
				{
				//	alert("Attachment id is not null and it is :::"+IdArray[1]);
					var rowNum = addDBDataInTableAttachment('tripdtls','encXML',new Array(tmpArray[0]),xmlPath[0],IdArray[1],'editRecordForDraft','deleteTravel','');
				}else
				{
	...
	//				var rowNum=addDBDataInTable('tripdtls','encXML',new Array(IdArray[0]),xmlPath[0],'editRecordForDraft','deleteTravel','');
					var rowNum = addDBDataInTableAttachment('tripdtls','encXML',new Array(tmpArray[0]),xmlPath[0],'','editRecordForDraft','deleteTravel','');
				}
...
//				var xmlDOM = populateAttachment(decXML,'frm1');
	...
	function submitRequest()
	{
		...
		var len=tb.rows.length;
 		...
 		if(NullCnt!=0)
 		{
	 		//	alert('Value is present ');
				...
				if(test())
   	 	     	...
   	 	     	{
   	 	     		alert("<fmt:message key="HRMS_Neccessary_Fields_req" bundle="${alertLables}" />");
   	 	     		//alert("Please Provide neccessary Fields");
   	 	     	}
 		}
 		else
 		{
 		//	alert('All rows are empty !!!!!! '); 			
 			...
 			{
 				 alert("<fmt:message key="HRMS_Onetour_req" bundle="${alertLables}" />");
 				//alert('Please Enter Atleast one Tour along With on Request');
 				return;
 			}
 		}
	...
	function departureTimeCheck(form)
	{
		...
		if(arrival_time.length==0)
		{
			...
			alert("<fmt:message key="HRMS_Arrival_time_req" bundle="${alertLables}" />");
		...
		var departure_date=depart_dt.value;
 		...
 		if(departure_date=="")
 		{
 			alert("<fmt:message key="HRMS_Depature_date_req" bundle="${alertLables}" />");
		...
		if(depart_tm.value=="")
		{
			alert("<fmt:message key="HRMS_time_req" bundle="${alertLables}" />");
			//alert("Please Insert Time");
		}else if(depart_tm.length==5 && depart_tm.charAt(2)!=':')
		{
			alert("<fmt:message key="HRMS_proper_time" bundle="${alertLables}" />");
		...
		}else if(depart_tm.length==3)
		{
			...
			{
				alert("<fmt:message key="HRMS_Special_Char_Notallowed" bundle="${alertLables}" />");
				//alert("cc");
				form.value="";
				return;
			}else
			{
				...
				if(imin>59)
				{
					alert("<fmt:message key="HRMS_Minutes_validaton" bundle="${alertLables}" />");
					//alert("Minute should not greater than 60 ");
					form.value="";
					return;
		...
		}else if(depart_tm.length==4)
		{
				...
				else
				{
					...
					if(ihour>25)
					{
						alert("<fmt:message key="HRMS_Hours_validation" bundle="${alertLables}" />");
						//alert("Hours could not be greater than 24 hours");
						form.value="";
						return;
					}else if(ihour==24)
					{
					...
					{
						alert("<fmt:message key="HRMS_Minutes_validaton" bundle="${alertLables}" />");
						//alert("Minute should not greater than 60 min");
						form.value="";
						return;
		...
		}else if(depart_tm.length==5 && depart_tm.charAt(2)==':')
		{
					...
					{
						alert("<fmt:message key="HRMS_Hours_validation" bundle="${alertLables}" />");
						//alert("Hours are not greater than 24 hours");
						form.value="";
						return;
					}else if(ihour==24)
					{
		...
		if(month==dmonth&&year==dyear&&day==dday)
		{
			...
			{
				alert("<fmt:message key="HRMS_Departuretime_arrival_time_validation" bundle="${alertLables}" />");
				//alert("Departure time is gerater than arrival time");
				form.value="";
				return;
	...
	function arrivalTimeCheck(form)
	{
		...
		if(arr_dt.value=="")
		{
			      alert("<fmt:message key="HRMS_Arrival_date_req" bundle="${alertLables}" />");
	...
	function departurechkdate(form)
	{
		...
		{
				alert("<fmt:message key="HRMS_Departure_place_validation" bundle="${alertLables}" />");
			//alert("Please select Previous Departure Place");
			form.value="";
			return;
		}
		...
		}else if(idyear==iayear)
		{
			...
			}else if(idmonth==iamonth)
			{
				if(idday<=iaday)
				{
					...
					alert("<fmt:message key="HRMS_Arrival_Date_validation" bundle="${alertLables}" />");
...
//		
	...
	function deleteRows() {
		...
		if(lCnt<=0)
		{
		    alert("<fmt:message key="HRMS_Select_row_validation" bundle="${alertLables}" />");
		     //alert("Please Select one row");
		      return;
		}
		if(lCnt>1)
		{
		 alert("<fmt:message key="HRMS_Row_validation" bundle="${alertLables}" />");
...
<hdiits:form name="frm1" validate="true" method="POST"
	...
	<table id='detailsTable' border="1" cellpadding="0" cellspacing="0"	style="display: none">
		...
		<tbody>
			<tr height="25%">
				...
				<td rowspan="2" colspan="1" height="25%" width="25%" align="center"><hdiits:dateTime
					name="arr_dt0" captionid="HRMS.DummyDeprDt" bundle="${caption}" maxvalue="31/12/2099 00:00:00" mandatory="true" /></td>
				<td rowspan="2" colspan="1" height="25%" width="25%" align="center"><hdiits:text
					name="arr_tm0" mandatory="false" id="arr_tm0" size="1" maxlength="5"
					onblur="arrivalTimeCheck(this)" /></td>
				<td rowspan="2" colspan="1" height="25%" width="25%" align="center">
				<hdiits:select name="depart_plc0" size="1" caption="departure_place"
					validation="sel.isrequired" id="depart_plc0" mandatory="false"
				...
				<td rowspan="2" colspan="1" height="25%" width="25%" align="center"><hdiits:text
					...
					onblur="departureTimeCheck(this)" mandatory="false" maxlength="5" /></td>
				<td rowspan="2" colspan="1" height="25%" width="5%"><hdiits:select
					name="con_mod0" id="con_mod0" caption="con_mode0" mandatory="false"
				...
				<td rowspan="2" colspan="1" height="25%" width="5%"><hdiits:select
					...
					mandatory="false" validation="sel.isrequired">
					<hdiits:option value="321611">------Select -----</hdiits:option>
				...
				<td rowspan="2" colspan="1" height="25%" width="5%"><hdiits:select
					name="accom0" id="accom0" caption="accomodation0" mandatory="false"
		...
		<tbody>
			<tr height="25%">
				...
				<td rowspan="2" colspan="1" height="25%" width="25%" align="center"><hdiits:dateTime
					name="arr_dt1" captionid="HRMS.DummyDeprDt" bundle="${caption}" maxvalue="31/12/2099 00:00:00" onblur="departurechkdate(this)"
					mandatory="true" /></td>
				<td rowspan="2" colspan="1" height="25%" width="25%" align="center"><hdiits:text
					size="1" name="arr_tm1" onblur="arrivalTimeCheck(this)"
					mandatory="false" maxlength="5" /></td>
				<td rowspan="2" colspan="1" height="25%" width="25%" align="center">
				<hdiits:select name="depart_plc1" size="1" caption="depart_place1"
					validation="sel.isrequired" id="depart_plc1" mandatory="false"
				...
				<td rowspan="2" colspan="1" height="25%" width="25%" align="center"><hdiits:dateTime
					name="depart_dt1" captionid="HRMS.DummyDeprDt" maxvalue="31/12/2099 00:00:00" bundle="${caption}" onblur="checkdate(this)"
					mandatory="true" /></td>
				<td rowspan="2" colspan="1" height="25%" width="25%" align="center"><hdiits:text
					size="1" name="depart_tm1" onblur="departureTimeCheck(this)"
					mandatory="false" maxlength="5" /></td>
				<td rowspan="2" colspan="1" height="25%" width="5%"><hdiits:select
					name="con_mod1" caption="con_mode1" mandatory="false"
				...
				<td rowspan="2" colspan="1" height="25%" width="5%"><hdiits:select
					name="purpose1" caption="purposeofstay1" mandatory="false"
				...
				<td rowspan="2" colspan="1" height="25%" width="5%"><hdiits:select
					name="accom1" caption="accomodation1" mandatory="false"
	...
	<br>
	<center><hdiits:button type="button" name="addButton"
	...
	<br>
	<br>
	<center><hdiits:button type="button" name="saveButton"
	...
	<center><hdiits:button type="button" name="closeButton"
		...
		onclick="resetTourDetails()" /></center>
	</div>
...
<c:forEach var="flagTypeVar" items="${flagType}">
	<script>
		document.frm1.arr_dt0.readOnly=true;
 		document.frm1.depart_dt0.readOnly=true;
 		document.frm1.arr_dt1.readOnly=true;
 		document.frm1.depart_dt1.readOnly=true; 
