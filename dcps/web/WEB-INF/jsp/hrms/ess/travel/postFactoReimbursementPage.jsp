...
<script language="JavaScript1.2">
	...
	travelAlertMsg[19]="<fmt:message bundle='${alertLables}' key='HRMS.touralongwithrequest'/>";
...
<hdiits:form name="frm1" validate="true" method="POST"
	...
	<ul id="maintab" class="shadetabs">
		<li class="selected"><a href="#" rel="tcontent1"> <fmt:message
			key="HRMS.PostFactoRmbs" bundle="${caption}" /> </a></li>
	</ul>
	</div>
...
<hdiits:fieldGroup titleCaptionId="HRMS.Journey_Details" bundle="${caption}" expandable="false"/>
		...
		<!-- First Row -->
		 ...
		 <tr class="odd">
			...
			<td rowspan="1" colspan="1" width="10%"><hdiits:text size="1"
				name="depart_tm${journeyRow.srNo}" classcss="mandatorycontrol"  disable="${journeyRow.viewFlag}" default="${journeyRow.departTime}" id="depart_tm${journeyRow.srNo}"  onblur="timeCheck(this);checkDates(this)"
	...
	<table id="btTable">
		...
		<c:if test="${editFlag eq true}">
			<hdiits:button type="button" name="updateBt" value="Update Details"
			id="updateBt" captionid="HRMS.Update" bundle="${caption}" onclick="updateTripDetails(this)" />
		</c:if>
		<c:if test="${NoOfRow gt 2}">
		
		<hdiits:button type="button" name="123"
			value="Delete Journey" id="123" captionid="HRMS.DeleteRow"
			onclick="deleteJourneyRow(this)" bundle="${caption}"  />
		</c:if>
		<br>
		<br>
	</table>
	...
	<c:if test="${not empty tripTableList}">
		<center>
			<table  border="1" id="tripTable"  cellpadding="1" cellspacing="1" width="80%" align="center"  >
					<tr  class="datatableheader">
						<td>
							<hdiits:caption captionid="HRMS.SRNO" bundle="${caption}"></hdiits:caption>
						</td>
						<td>
							<hdiits:caption captionid="HRMS.TourName" bundle="${caption}"></hdiits:caption>
						</td>
						<td colspan="3">
							<hdiits:caption captionid="HRMS.Action" bundle="${caption}"></hdiits:caption>
						</td>	
					</tr>
					<c:forEach items="${tripTableList}" var="tripTableVO">
						<tr>
							...
							<td colspan="1">
								<a href="#" id="editrowId${RowCnt}" onclick="editTripDetail(this)">Edit</a>
							</td>
							<td colspan="1">
								<a href="#" id="deleterowId${RowCnt}" onclick="deleteTripDetail(this)">delete</a>
							</td>
	...
	<table>
	 <c:forEach items="${travelExpList}" var="ExpList">
		...
		<hdiits:hidden name="${ExpList.cmnLookupMst.lookupName}Id" default="${ExpList.cmnLookupMst.lookupId}"/>	 
			...
			<td>
				<hdiits:number size="5" name="${ExpList.cmnLookupMst.lookupName}" default="${ExpList.charge}" />
			</td>
			<td>
	...
	<table id="cntButton">
		<hdiits:button type="button" name="saveButton" value="Submit Request"
			id="SaveButton" captionid="HRMS.Submit" bundle="${caption}" onclick="submitTravelRmbsReq(this)" />
		<hdiits:button type="button" name="saveasButton" value="Add Trip"
			id="saveasButton" captionid="HRMS.AddTrip"  bundle="${caption}" onclick="addTripInTable(this)" />
		<hdiits:button type="button" name="closeButton"  bundle="${caption}" value="Close"
			id="closeButton" captionid="HRMS.SaveAsDraft" onclick="closewindow(this)" />
