...
<hdiits:form method="POST" name="frmSummaryPage" validate="true">
	...
	</c:set> <display:table name="${DDOLST}" requestURI="" pagesize="10"
		...
		Amount"
			...
			<display:column class="tablecelltext" title="Reject"
				...
				<input type="button" id="btnReject${srno}" name="btnReject${srno}" value="Reject"
					onclick='rejectPayBill(lbl${srno}, ah${srno})' />
			</display:column>
...
function rejectPayBill(ddoCode, billno){
	...
	if(answer){
		var url = "ifms.htm?actionFlag=rejectPaybillByLevelThree&paybillId="+billId+"&ddocode="+ddocode+"&hdMonth="+month+"&hdYear="+year+"&schemeCode="+schemeCode;
...
function GoToPage(){
	document.forms[0].action = "./ifms.htm?actionFlag=ViewFinalJsp";
