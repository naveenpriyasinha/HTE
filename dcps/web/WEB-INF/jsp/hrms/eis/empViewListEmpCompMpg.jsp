...
function chkValue()
{
	...
	var empId="";
	if(document.getElementById("Employee_ID_EmpInfoSearch").value==document.getElementById("Employee_Name_EmpInfoSearch").value)
	{
...
//	window.close();	
	var href = "./hrms.htm?actionFlag=getEmpDataForMapping&empId="+valueempid+"&edit=Y";
	objChildWindow = window.open(href,"Show","toolbar=no, location=no, directories=no,status=yes, menubar=no, scrollbars=yes, resizable=yes, top=10, left=10, width=980, height=660, copyhistory=no");
}

function AllDdctBulk()
{
	//alert('AllDdctBulk');
		url = 'ifms.htm?actionFlag=getBulkAllwDdct';
		document.EmpInfo.action = url;
		document.EmpInfo.submit();
...
<li class="selected"><a href="#" rel="tcontent1"><b><fmt:message key="EmpCompMpg.Head" bundle="${commonLables}"/></b></a></li>
...
<br/><br/>
 <div id="tcontent1" style="background-color: #E8E3E3;border-style: inset;border-color:#B24700 ;border-width: thin" > 
	  ...
	  <br>
	<hdiits:form name="EmpInfo" validate="true" method="POST"	action="./hrms.htm?actionFlag=getEmpDataForMapping" encType="text/form-data">
		<c:set value="display:none" var="displayStyle"/>
	<c:if test="${empAllRec!='true'}">
		<c:set value="display:show" var="displayStyle"/>
		</c:if>
		
	<div id="tcontent3" style="background-color: #E8E3E3;" >	
	<table>
		<tr>
			<td><a style="color: black;" href="javascript:AllDdctBulk()">Emp. Elligibility for Allwances/Deductions in Bulk</a></td>
		</tr>
	</table>
	</div>
 ...
 <c:set value="${row.empId}" var="otherIdForLink"/>
	  ...
	  <display:column class="tablecelltext" title="Employee Name" headerClass="datatableheader" style="text-align: center;font-size:12px;"> 
	  		 <a href="./hrms.htm?actionFlag=getEmpDataForMapping&empId=${row.empId}&edit=Y&elementId=9000218" id="otherLink${row.empId}">${row.empName}</a>
	  		 
	</display:column>	
	
<%-- 	  <c:choose> --%>
<%-- 	  	<c:when test="${row.dcpsOrGpf eq contract }"> --%>
<%-- 	  		<c:set var="dcpsOrGpf" value="GPF"/> --%>
<%-- 	  	</c:when> --%>
<%-- 	  	<c:when test="${row.empType eq fixed }"> --%>
<%-- 	  		<c:set var="dcpsOrGpf" value="Dcps"/> --%>
<%-- 	  	</c:when> --%>
<%-- 	  	</c:choose> --%>
  <display:column class="tablecelltext" title="DCPS/GPF"  headerClass="datatableheader" style="text-align: center;font-size:12px;"><c:out value="${row.dcpsorgpf}"/></display:column>
	  <c:choose>
...
<%-- 				</display:column> --%>
  	  <display:column class="tablecelltext" title="Employee Type"  headerClass="datatableheader" style="text-align: center;font-size:12px;"><c:out value="${empType}"/></display:column>
