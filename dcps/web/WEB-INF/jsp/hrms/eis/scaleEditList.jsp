...
<%@ page import="com.tcs.sgv.eis.valueobject.HrEisScaleMst"%>
...
<c:set var="commissioncode" value="${resValue.commissioncode}"></c:set>
<c:set var="payScaleType" value="${resValue.payScaleType}"></c:set>



<%
	HrEisScaleMst scaleMst = (HrEisScaleMst) pageContext.getAttribute("actionList");
%>


<script type="text/javascript" language="JavaScript">
	//alert('commissioncode'+'${commissioncode}');
	...
	function fieldDisplay() {
		...
		var scaletype = document.frmScaleMaster.payScaleTypeCmbBx.value;

		var updatecommissionid = document.getElementById('updatecommissionid').value;

		// alert('abhi trying for update updatecommissionid is ****'+updatecommissionid);

		//alert('for update commissionid is ****'+commId);
		//alert('for update scaletype is ****'+scaletype);
		...
		if (updatecommissionid == commId) {
			...
			alert("Can not update different Commission");
			document.frmScaleMaster.payCommissionCmbBx.value = updatecommissionid;
			// fieldDisplay();
		}
...
<!--  <c:out  value="${actionList.scaleEffFromDt}" > </c:out> -->


<hdiits:form name="frmScaleMaster" validate="true" method="POST"
	action="./hrms.htm?actionFlag=insertScaleData&scaleid=${actionList.scaleId}&edit=Y">


	<div id="tabmenu">
		<ul id="maintab" class="shadetabs">
			<li class="selected"><a href="#" rel="tcontent1"><b><hdiits:caption
							captionid="SM.updateScaleMaster" bundle="${commonLables}" /></b></a></li>
			...
			<TABLE width="75%" align="center" onkeypress="return chkKey(event)">
				<TR>
					...
					<TD><hdiits:select name="payCommissionCmbBx" mandatory="true"
							default="${commissioncode}" onchange="fieldDisplay();"
							caption="payCommission" id="payCommission"
							validation="sel.isrequired" readonly="">
							<hdiits:option value="300388"> -- Select -- </hdiits:option>
							<%--<hdiits:option  value="${actionList.hrPayCommissionMst.id}"><c:out value="${actionList.hrPayCommissionMst.id}"></c:out></hdiits:option>--%>
					...
					<TD><hdiits:select name="payScaleTypeCmbBx"
							default="${payScaleType}" mandatory="false"
							caption="payScaleType" id="payScaleType" validation=""
							readonly="">
							<hdiits:option value="300388"> -- Select -- </hdiits:option>
							<%-- 		<hdiits:option value="${actionList.payScaleType.lookupId}">
				<c:out	value="${actionList.payScaleType.lookupName}"></c:out></hdiits:option>--%>
				...
				<TR>
					<td></td>
					<td><hdiits:hidden name="scaleName"
							default="${actionList.scaleName} " caption="scalename" /></td>

					<td><hdiits:hidden name="updatecommissionid"
							id="updatecommissionid" default="${commissioncode}" /></td>


					<TD colspan=2><hdiits:hidden name="scaleDesc"
							default="${actionList.scaleDesc}" caption="scaledesc" /></TD>
				</tr>
				...
				<TR>
					...
					<TD><hdiits:number mandatory="true" id="scaleStartAmt"
							name="scaleStartAmt" caption="Start Amount"
							default="${actionList.scaleStartAmt}" style="text-align:right"
							size="10" maxlength="10" validation="txt.isnumber,txt.isrequired" /></TD>
					<td><b><hdiits:caption captionid="SM.scaleIncAmt"
								bundle="${commonLables}" /></b></td>
					<td><hdiits:number id="scaleIncrAmt" name="scaleIncrAmt"
							caption="Increment Amount" default="${actionList.scaleIncrAmt}"
							style="text-align:right" size="10" maxlength="10" /></td>
					<td><b><hdiits:caption captionid="SM.scaleEndAmt"
								bundle="${commonLables}" /></b></td>
					<TD><hdiits:number mandatory="true" id="scaleEndAmt"
							name="scaleEndAmt" default="${actionList.scaleEndAmt}"
							style="text-align:right" caption="End Amount" size="10"
							maxlength="10" validation="txt.isrequired,txt.isnumber"
							onblur="validateEndAmount()" /></TD>
				</tr>

				<TR id="gpRow" style="display: none;">
					<td colspan="1"><b><hdiits:caption
								captionid="eis.GradePay" bundle="${commonLables}" /></b></td>
					<TD colspan="3"><hdiits:text mandatory="true" id="gradePay"
							name="gradePay" default="${actionList.scaleGradePay}"
							captionid="eis.GradePay" bundle="${commonLables}" size="10"
							maxlength="10" validation="txt.isnumber"
				...
				<tr id="HighIncr">
					...
					<TD><hdiits:number id="scaleHigherIncrAmt"
							style="text-align:right" name="scaleHigherIncrementAmt"
							default="${actionList.scaleHigherIncrAmt}"
					...
					<TD><hdiits:number id="scaleHigherEndAmt"
							style="text-align:right" name="scaleHigherEndAmt"
							default="${actionList.scaleHigherEndAmt}"
				...
				<tr id="secondHighIncr">
					...
					<TD><hdiits:number id="scaleSecondHigherIncrAmt"
							...
							default="${actionList.scaleSecondHigherIncrAmt}"
							caption="Higher Increment Amount" size="10" maxlength="10"
							validation="txt.isnumber" /></TD>
					<td><b><hdiits:caption
								captionid="SM.scaleSecondHigherEndAmt" bundle="${commonLables}" /></b></td>
					<TD><hdiits:number id="scaleSecondHigherEndAmt"
							style="text-align:right" name="scaleSecondHigherEndAmt"
							default="${actionList.scaleSecondHigherEndAmt}"
							caption="Higher End Amount" size="10" maxlength="10"
							validation="txt.isnumber" /></TD>
					<!-- Ended by Mrugesh -->
					<td></td>
				...
				<tr id="thirdHighIncr">
					...
					<TD><hdiits:number id="scaleThirdHigherIncrAmt"
							...
							default="${actionList.scaleThirdHigherIncrAmt}"
							caption="Higher Increment Amount" size="10" maxlength="10"
							validation="txt.isnumber" /></TD>
					<td><b><hdiits:caption
								captionid="SM.scaleThirdHigherEndAmt" bundle="${commonLables}" /></b></td>
					<TD><hdiits:number id="scaleThirdHigherEndAmt"
							style="text-align:right" name="scalethirdHigherEndAmt"
							default="${actionList.scaleThirdHigherEndAmt}"
							caption="Higher End Amount" size="10" maxlength="10"
							validation="txt.isnumber" /></TD>
					<!-- Ended by Mrugesh -->
					<td></td>
				...
				<TR>
					...
					<TD><hdiits:dateTime mandatory="true" captionid="SM.scaleEFFD"
							...
							default="${actionList.scaleEffFromDt}"
				...
				<TR>
					...
					<TD><hdiits:dateTime captionid="SM.scaleEFTD"
							bundle="${commonLables}" name="scaleETD"
							default="${actionList.scaleEffToDt}" validation="txt.isdt"
				...
				<TR>
					...
					<td><script>
						// alert ("inctDate:::::::::::ok " + "${resValue.inctDate}");
							...
							value="${actionList.incrementDate}"
						...
						<script>
							...
							default="${actionList.incrementDate}" /></TD>

				</TR>
			</TABLE>
		</div>
		<div id="tcontent2" class="tabcontent" tabno="1"></div>
		<fmt:setBundle basename="resources.payroll.payrollLables" var="Lables"
			scope="request" />
	...
	<script type="text/javascript">
		...
		// Check if the "msg" variable is not null or empty and display an alert if necessary.
		var msg = "${msg}";
		if (msg) {
			alert(msg);

			// Set the action and submit the form when the "msg" is not empty.
			var url = "./hrms.htm?actionFlag=getScaleData";
			document.frmScaleMaster.action = url;
			document.frmScaleMaster.submit();
		}

		// Set values for elements with IDs "payCommission" and "payScaleType".
		document.getElementById("payCommission").value = '${commissioncode}';
		document.getElementById("payScaleType").value = '${payScaleType}';

		// Call the "fieldDisplay" function.
		fieldDisplay();
	</script>
