...
<c:set var="treasuriesList" value="${resValue.ListTreasuriesUnMatchEntries}"></c:set>
...
function getStartAndEndDateForYearUnMatched()
{
...
function getUnMatchedContriEntriesReport(treasuryCodeValue)
{
	...
	url = "ifms.htm?actionFlag=reportService&reportCode=700030&action=generateReport&treasuryCode="+treasuryCode
		+"&yearDesc="+yearDesc+"&fromDate="+fromDate+"&toDate="+toDate+"&yearId="+yearId+"&MatchedOrUnMatched=UnMatched";

	self.location.href = url ;
}
function getUnMatchedEntries()
{
	...
	{
		self.location.href='ifms.htm?actionFlag=loadUnMatchContriEntryForm&elementId=700073&yearId='+yearId+'&fromDate='+fromDate+'&toDate='+toDate;
	}
}
</script>

<fmt:setBundle	basename="resources.dcps.dcpsLabels" var="DCPSLables" scope="request" />
<hdiits:form name="frmDCPSUnMatchedEntries" encType="multipart/form-data"
...
<table id="table1" width="70%" align="center" cellpadding="4" cellspacing="4">	
	<tr>
		...
		<td colspan="3" align="left">
			<select name="cmbFinYear" id="cmbFinYear" style="width:25%" onchange="getStartAndEndDateForYearUnMatched();">
	...
	<center>
			<hdiits:button type="button" captionid="BTN.SUBMIT" bundle="${DCPSLables}" id="submit" name="submit" onclick="getUnMatchedEntries();"></hdiits:button>
	</center>
<br/>
...
<c:if test="${treasuriesList != null}">
	...
	<display:table list="${treasuriesList}" id="vo" cellpadding="4" pagesize="10" export="false" partialList="true"  offset="1" excludedParams="ajax" sort="external" size="${resValue.totalRecords}" defaultsort="3" defaultorder="descending" >    			      			        			      			      			  		         		         				
			              ...
			              <display:column  style="text-align:left" titleKey="CMN.TreasuryName" headerClass="datatableheader" class="oddcentre"
			               >	
			              <a href = # onclick="getUnMatchedContriEntriesReport('${vo[0]}');"><c:out value="${vo[1]} - ${vo[0]}" /></a>		      
				          </display:column>
