...
//	    	col1<%=request.getParameter("attachmentName")%>.innerHTML=document.forms[formNameGlobalVar<%=request.getParameter("attachmentName")%>].desc<%=request.getParameter("attachmentName")%>.value;
	    ...
	    col3<%=request.getParameter("attachmentName")%>.innerHTML='<a class="removeLink" onClick="delRow<%=request.getParameter("attachmentName")%>(this,'+rowNumber + ')" target="target_upload<%=request.getParameter("attachmentName")%>">Remove</a> '; 			
		<%}else{%>
...
function viewRow<%=request.getParameter("attachmentName")%>(src,rowNumber)
{
	...
	try
	{
		...
		// Row Count added for Attachment Multiple Add/Edit functionality 2007-10-11
		var rowCount = dbRowCount<%=request.getParameter("attachmentName")%>;
  		var urlValue = "ifms.htm?actionFlag=viewAttachmentForDCPS&attachmentUniqeName=<%=request.getParameter("attachmentName")%>&rowNumber="+rowNumber+"&rowCount="+rowCount+"&rowIndex=";
...
function insRowEdit<%=request.getParameter("attachmentName")%>()
{
		...
		if(cmnAttachmentMst != null)
		{
		    ...
		    %>
			<%if(removeAttachment!=null && removeAttachment.trim().length()>0 && removeAttachment.equalsIgnoreCase("Y") && User.equals("ZPDDOAsst") ){%>		    
			...
			<%}else{%>
		    col3<%=request.getParameter("attachmentName")%>.innerHTML='';
		    <%}%>
...
if(!(attachmentTitle.equals("Existing")))
{
