...
<% try{ %>
        ...
        <form name="actiondetails" method="POST" action="ifms.htm" onsubmit="return validate_form(this)" method="post">
...
<div id="progressImage" style="display:none"></div>
    ...
    <c:forEach var="cnfg" items="${configList}">
		...
		<% x = x+1; %>
            ...
            <td><input type="text" name="actiondesc" id="0" value='<c:out value="${cnfg[1]}"/>'></td>
            <td align="left" class="tabnavtd" id="tabnavtd"></td>
       </tr>

		<input type="hidden" name="actionid" value='<c:out value="${cnfg[2]}"/>'>
		<input type="hidden" name="createddate" value='<c:out value="${cnfg[3]}"/>'>
		<input type="hidden" name="createdempid" value='<c:out value="${cnfg[4]}"/>'>
