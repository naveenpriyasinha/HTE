...
var flagForUpdatedVO = false;
	...
	function addDataInTable(tableId, hiddenField, displayFieldArray, editMethodName, deleteMethodName, viewMethodName,methodName) 
	{
//		alert ("addDataInTable called...");
		...
		trow.insertCell(0).innerHTML = "<INPUT type='hidden' name='" + hiddenField + "' id='" + hiddenField + counter + "' value='" + methodName + "'/>";				
...
//			alert ("EXCEPTION THROWN ");
						...
						xmlFilePath,editMethodName,deleteMethodName,viewMethodName)
	{
		...
		trow.cells[0].style.display = 'none';
		trow.insertCell(1).innerHTML ="<INPUT type='radio' name='FirstCheck' id='FirstCheck' value='YesChecked"+counter+"' onclick=showchecked('" + trow.id + "','" + counter +"')>";
...
//			alert ("EXCEPTION THROWN ");
	...
	//alert('hFieldId='+hFieldId);
		...
		//trow.cells[0].style.display = 'none';
		trow.cells(0).innerHTML ="<INPUT type='radio' name='FirstCheck"+counter+"' id='FirstCheck"+counter+"' value='YesChecked"+counter+"')>";
		var len = displayFieldArray.length;
		for(var i = 0; i < len; i++)
		{
			var field = document.getElementById(displayFieldArray[i]);
