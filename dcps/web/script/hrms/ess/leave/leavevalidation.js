...
var cmbLeaveArr= new Array();// to store combination leave when coming back from server.
function goToMainPage(frm){
frm.action="hdiits.htm?actionFlag=getHomePage";
frm.method="POST";
frm.submit();
}
function  checkFirstHalfDayJoining(){
//checkHalfDay2();
 ...
 var  natureofleave=document.frmleaveapply.natureofleave.value.split('_');
	if(document.forms[0].fromdate.value!="" &&  document.forms[0].todate.value!=""){
        	...
        	if(document.forms[0].halfday[0].status){

				if(document.forms[0].fromdate.value == document.forms[0].todate.value){
						document.getElementById('before2_lbl_joining').style.display="";
						document.getElementById('before2_radio_joining').style.display="";
						document.getElementById("joining_halfday").style.display="";						
						document.getElementById('firstdayhalfdayNo').status=true; 
						document.getElementById('lastdayhalfdayNo').status =true;
						document.getElementById('First_Day_joining').style.display="none";
						document.getElementById('Last_Day_joining').style.display="none";
						document.getElementById("firstday_halfday_joining").innerHTML="";
						document.getElementById("lastday_halfday_joining").innerHTML="";
				}

				else if( document.forms[0].fromdate.value != document.forms[0].todate.value){

				document.getElementById('First_Day_joining').style.display="";
				document.getElementById('Last_Day_joining').style.display="";
				document.getElementById("firstday_halfday_joining").innerHTML="<b>" + labels[8] +" "+ document.forms[0].fromdate.value +"</b>";
				document.getElementById("lastday_halfday_joining").innerHTML="<b>" +labels[8] +" "+ document.forms[0].todate.value +"</b>";
				document.getElementById('before2_lbl_joining').style.display="none";
				document.getElementById('before2_radio_joining').style.display="none";
				...
				if(document.getElementById('firstdayhalfdayYes').status && document.getElementById('lastdayhalfdayYes').status && document.getElementById('halfdayYes').status){
					document.getElementById('before2_radio_firstday_joining').innerHTML="<b>" +labels[6] + "</b>";
					document.getElementById('before2_radio_lastday_joining').innerHTML="<b>"+labels[7]+ "</b>";
				}
					...
					document.getElementById('before2_radio_firstday_joining').innerHTML="<b>"+labels[6]+"</b>";
					document.getElementById('before2_radio_lastday_joining').innerHTML="";
				}
				else if(!document.getElementById('firstdayhalfdayYes').status && document.getElementById('lastdayhalfdayYes').status && document.getElementById('halfdayYes').status){
					document.getElementById('before2_radio_firstday_joining').innerHTML="";
					document.getElementById('before2_radio_lastday_joining').innerHTML="<b>"+labels[7]+"</b>";
				}
				else if(!document.getElementById('firstdayhalfdayYes').status && !document.getElementById('lastdayhalfdayYes').status && document.getElementById('halfdayYes').status){
					document.getElementById('before2_radio_firstday_joining').innerHTML="";
		...
		showOrd(document.frmleaveapply.natureofleave);
				...
				/*document.getElementById("halfdayNo_joining").status=true;*/
...
if((arrData[0]=="1_1_1" || arrData[0]=="1_1_2") && arrData[3]==eval(0.5))
{
 document.getElementById("halfday").style.display="";
 if(window.opener.document.forms[0].elements["halfday_comb"].value==eval(1)){
 document.forms[0].halfday[0].status=true;
 
if(window.opener.document.forms[0].elements["before2_comb"].value==eval(1)){
document.forms[0].before2[0].status=true;
}
else{
document.forms[0].before2[1].status=true;
}
 }
 else{
 document.forms[0].halfday[1].status=true;
 }
...
document.forms[0].natureofleave.options[k].selected=true;
break;
}
}
document.forms[0].fromdate.value=arrData[1];
document.forms[0].todate.value=arrData[2];
 ...
 document.forms[0].Submit.disabled=false;
	...
	function validateForm(frm,alertMsgDates,requiredMsg,components,holidayList){
		...
		if(frm.prefix[0].status==true){
			if(!validate('RDate',"$CPTN" + " " + requiredMsg[1],'prefixfromdate',components[1])){
				return false;
			}
			if(!validate('RNull',"$CPTN" + " " + requiredMsg[0],'prefixfromdate',components[1])){ 
				return false;
			}
			if(!validate('RDate',"$CPTN" + " " + requiredMsg[1],'prefixtodate',components[2])){ 
				return false;
			}
			if(!validate('RNull',"$CPTN" + " " + requiredMsg[0],'prefixtodate',components[2])){ 
				return false;
			}
			else{
		...
		if(frm.suffix[0].status==true){
			...
			if(!validate('RDate',"$CPTN" + "" + requiredMsg[1],'suffixfromdate',components[3])){ 
				return false;
			}
			if(!validate('RNull',"$CPTN" + "" + requiredMsg[0],'suffixfromdate',components[3])){ 
				return false;
			}
			if(!validate('RDate',"$CPTN" + "" + requiredMsg[1],'suffixtodate',components[4])){ 
				return false;
			}
			if(!validate('RNull',"$CPTN" + "" + requiredMsg[0],'suffixtodate',components[4])){ 
				return false;
			}
			else{
						if(frm.combinationleave[0].status==false){
		...
		prefixStatus=true;
			...
			if(frm.combinationleave[0].status==false){
				if(!validate('RDropMenu',"$CPTN" + " " + requiredMsg[2],'natureofleave',components[0])){ 
					return false;
			}
			if(!validate('RDate',"$CPTN" + " " + requiredMsg[1],'fromdate',components[6]))
			{ 
			return false;
			}
			 
			if(!validate('RNull',"$CPTN" + " " + requiredMsg[0],'fromdate',components[6]))
			{ 
			return false;
			}
			 
			if(!validate('RDate',"$CPTN" + " " + requiredMsg[1],'todate',components[7]))
			{ 
			return false;
			}
			 
			if(!validate('RNull',"$CPTN" + " " + requiredMsg[0],'todate',components[7]))
			{ 
			return false;
			}
...
//					document.getElementById('no1').style.display='';
	...
	//				document.getElementById('no2').style.display='';
		...
		//			document.getElementById('no2').style.display='none';
					...
					if(leaveDetail[0].childNodes[18].text!="-"){
...
//						document.getElementById("first_second_Day").style.display="none";
	...
	//					document.getElementById("halfday").style.display="";
					checkFirstHalfDay();
						...
						cmbLeaveArr[k]=leave_dtl;
						//addRow(0);
						document.getElementById("lvdtl").style.display="";
						addRow_display(0,"",cmbLeaveArr);
		...
		//				addRow(0,document.getElementById("lvdtl"),2);		
				...
				if(cmbLeaveArr.length>0){
					addRow(0,cmbLeaveArr);
				}
			disp_counter=1;
...
function populateDataForModifyInJoining(xmlDoc,frm) 
	{ 
			 document.forms[0].Submit.disabled=false;
			//window.location.reload(true);
			//ajaxfunctionLeaveAttachment();
...
//					document.getElementById('no2').style.display='none';
					...
					if(leaveDetail[0].childNodes[19].text!="-"){
...
function populateRecordInReqPage(frm,xmlDoc){
					...
					var xmlDOM=getDOMFromXML(lineBreak(xmlDoc));	
					...
					temp+=document.getElementById(rows[m]).cnt+"~";
						for(l=0;l<leaveArr.length;l++){
							if(document.getElementById(rows[m]).cnt==leaveArr[l]){
									...
									leaveDataArrParent.splice(l,1);
									combFromArr.splice(l,1);
									combToArr.splice(l,1);
								}
							}
							counter-=1;
							delCounter+=1;	
...
//						document.getElementById("first_second_Day").style.display="none";
	...
	//					document.getElementById("halfday").style.display="";
					...
					if(leave_other_dtl[k].childNodes[2].text!="-"){
					FormatDDMMYYYY(leave_other_dtl[k].childNodes[2].text,document.getElementById("sancFrom_"+componentIds));
					}else
					{
						document.getElementById("sancFrom_"+componentIds).innerHTML='-';
					}
					if(leave_other_dtl[k].childNodes[3].text!="-"){
						FormatDDMMYYYY(leave_other_dtl[k].childNodes[3].text,document.getElementById("sancTo_"+componentIds));
...
function populateAttachmentForEditing(decXML,attachmentTable)
{
							
															//LeaveAttachment
		deleteAllRowsFromTable(document.getElementById('myTable'+attachmentTable));
...
{ 
		...
		if(eval(z)==eval(0)){
			...
			inp1.setAttribute("id","chk"+lvdtlArr[0]); 
...
//			document.forms[0].nol.value=lvdtlArr[0];
			selectedIndexOfCombo(document.forms[0].nol,lvdtlArr[0]);
			cell3.innerHTML = document.forms[0].nol.options[document.forms[0].nol.selectedIndex].innerHTML; 
			...
			//document.getElementById("totalDays").innerHTML=eval(document.getElementById("totalDays").innerHTML)+eval(lvdtlArr[3]);
			row.appendChild(cell1); 
			row.appendChild(cell2); 
			row.appendChild(cell3); 
			row.appendChild(cell4); 
			...
			row.appendChild(cell6)
			//row.style.display="none";
			row.setAttribute("id","rowchk"+lvdtlArr[0]);
...
for (i=1;i<component.options.length;i++){
...
function lineBreak(strMultiLineText){
var strSingleLineText = strMultiLineText.replace(/<br><br>/g,"\n"); 
