...
$('#example').append('<caption style="caption-side: top">HTESevaarth DCPS Legacy Data Status Report</caption>');
...
<img src="images/mahasevaarth.png" id="image" name="image" crossorigin="anonymous" style="display:none;" >
	<!-- Added Brijoy  -->
        ...
        <div class=" logo-mainn">
            <div class="row logo-row">
                <div class="col-md-9 flex">
                    <div class="logobar ">
                        <a class="logo" href="#"><img src="images/logo-v16.png" class="logo-img"></a>
                    </div>
                    <h5 class="logo-Title">
...
<h2 align="center"><font><strong>HTESevaarth DCPS Legacy Data Status Report</strong></font></h2>
...
String sql ="SELECT ddo.OFF_NAME||'('||ddo.DDO_CODE||')' as DDO_OFF,  sum(decimal(bh.BH_EMP_AMOUNT,20,2))+sum(decimal(bh.BH_EMP_AMOUNT,20,2)) as TOTAL_AMT,  case when bh.file_status = 0 then cast('File not Validated' as varchar(20)) when bh.file_status = 1 then cast('File is validated' as varchar(20))   when bh.file_status = 2 then cast('File is rejected' as varchar(20)) when bh.file_status = 5 then cast('Contribution file send' as varchar(25)) when bh.file_status =11 then cast('File send and get Challan' as varchar(25)) end as File_status  FROM ifms.NSDL_BH_DTLS bh  inner join ifms.RLT_ZP_DDO_MAP as rlt on rlt.ZP_DDO_CODE=bh.DDO_CODE inner join ifms.MST_DCPS_DDO_OFFICE as ddo on ddo.DDO_CODE=rlt.REPT_DDO_CODE where bh.STATUS <> -1 and   bh.file_status in  (5,11)  group by  ddo.OFF_NAME||'('||ddo.DDO_CODE||')',FILE_STATUS";
...
%>
<td><%=srno++%></td>
<td><%=resultSet.getString("DDO_OFF") %></td>
<td><%=resultSet.getString("File_status") %></td>
