...
public class AISReportDAO extends DefaultReportDataFinder 
implements ReportDataFinder,ReportEventListener
{
	...
	{
		logger.info("==> in AISReportDAO findReportData...");
		...
		try{
			...
			//Added by abhilash	
			 ...
			 GPFReportDAO reportDAO = new GPFReportDAOImpl(HrPayPaybill.class);
				...
				//System.out.println("loanID is**********"+loanID);
				 ...
				 if(loanID==64 || loanID==65)
				 {
			 RowList = reportDAO.getLoanDetailsForAIS(loanID,Long.valueOf(BillNo),month,year,locId);
				 }
				 else if(loanID==71 || loanID==69)
				 {
					 RowList = reportDAO.getLoanDetailsForAISInt(loanID,Long.valueOf(BillNo),month,year,locId);
				 }
			 String row0="";
			 long row1=0;
			 long row2=0;
			...
			logger.info("totalIncomeTax********" + totalAmtDedun);
			 ...
			 if(loanID==64)
			 {
				 deptHeader="Schedule for the Recovery of H.B.A TO AIS"+System.getProperty("line.separator")+"From Major Head 7610";
			 }
			 else if(loanID==65)
			 {
				 deptHeader="Schedule for the Recovery of M.C.A TO AIS"+System.getProperty("line.separator")+"From Major Head 7610";
			 }
			 else if(loanID==71)
			 {
				 deptHeader="Schedule for the Recovery of H.B.A TO AIS"+System.getProperty("line.separator")+"From Major Head 0049";
			 }
			 else if(loanID==69)
			 {
				 deptHeader="Schedule for the Recovery of M.C.A TO AIS"+System.getProperty("line.separator")+"From Major Head 0049";
			 }
	...
	public ReportVO getUserReportVO( ReportVO report, Object criteria )
	{
		...
		if(  report.getReportCode().equals("8000059") )
