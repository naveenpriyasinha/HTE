...
public class GPFAbstractSubReportDAOImpl extends DefaultReportDataFinder 
implements ReportDataFinder,ReportEventListener 
{
	...
	public Collection findReportData( ReportVO report, Object criteria ) throws ReportException
	{
		...
		report.setStyleList(URLStyleVO);
			...
			dataStyle = new StyledData();
			dataStyle.setStyles(leftBoldStyleVO);
			dataStyle.setData("1.Certified that the name, amounts of individuals deductions & the total shown in column (7) have been checked by reference to");
			dataStyle.setColspan(2);
			row.add(dataStyle);
			DataList.add(row);
			...
			dataStyle = new StyledData();
			dataStyle.setStyles(leftBoldStyleVO);
			dataStyle.setData("the bill, vide. paragraph 224 of the Audit Manual.");
			dataStyle.setColspan(2);
			row.add(dataStyle);
			DataList.add(row);
