...
public class RevenueStampReportAO extends DefaultReportDataFinder implements ReportDataFinder
{
	...
	public Collection findReportData( ReportVO report, Object criteria ) throws ReportException
	{
		...
		try   
		{
			...
			if(report.getReportCode().equals("5000099"))
			{
				...
				String Title = "Schedule for the Recovery of Revenue Stamp";
				...
				*/
				...
				query+=" dsgn.dsgnName,pr.proofNum,paybill.grossAmt,paybill.deduc9570,paybill.deduc9135";
				...
				query+=" and head.hrPayPaybill=paybill.paybillGrpId and paybill.cmnLocationMst.locId="+locationId+" and paybill.deduc9135>0 order by concat(emp.empLname,' ',emp.empFname,' ',emp.empMname)";
				
				
				//System.out.println("***Query for Deduction details**" +query);
				Query sqlQuery = hibSession.createQuery(query);	    
				logger.info("Shivram Test "+sqlQuery);
				...
				if(RowList.size()!=0)
				{
					...
					int pageCnt=1;
						...
						while (itr.hasNext())
						{
							...
							double revenueStamp = Double.parseDouble((rowList[6]!=null?rowList[6]:"0").toString());
				...
				row.add(proofNumber);*/
				row.add(grossAmount);
				row.add(revenueStamp);
				
				totalIncomeTax+=revenueStamp;
