...
public class UserPostDAO extends GenericDaoHibernateImpl<UserPostCustomVO, Long>  {
	...
	public List getAllUserPostRltDatabyDesg(long loc_id,long desgId)
	{
	    ...
	    //added to not display already attached post
	    sb.append(" and pd.POST_ID not in (select DISTINCT post_id from RLT_DCPS_PAYROLL_EMP where POST_ID is not null) ");
	...
	public List getPostName(long locId){
		...
		//public List getPostNameForDisplay(long locId,String lPostName,String PsrNo,String BillNo,String Dsgn ){//sunitha
	//added by roshan	
/*	public List getPostNameForDisplay(String locId,String lPostName,String PsrNo,String BillNo,String Dsgn ){//sunitha
		List postNameList = new ArrayList();
		Session hibSession = getSession();
		StringBuffer sb = new StringBuffer();
		...
		sb.append("  where pd.loc_id in (" + locId +") and P.POST_ID = PD.POST_ID and  pd.dsgn_id = ds.dsgn_id and ds.lang_id = 1  " );
		if(PsrNo != null && !(PsrNo.trim()).equals(""))
	...
	//added by roshan
	public List getPostNameForDisplay(String locId,String lPostName,String PsrNo,String BillNo,String Dsgn, String ddoSelected ){//sunitha
		List postNameList = new ArrayList();
		Session hibSession = getSession();
		StringBuffer sb = new StringBuffer();
		sb.append("  select pd.post_name,pd.post_id,(select concat(concat(concat(o.emp_fname, ' '), concat(o.emp_mname, ' ')),concat( o.emp_lname,' ')) from org_emp_mst o, org_userpost_rlt up ");
	...
	//	{
			sb.append("  , (select mp.ddo_code  from org_ddo_mst mp where  p.loc_id = mp.location_code) BillNo,org.post_type_lookup_id,cmn.lookup_name from org_post_details_rlt pd, org_designation_mst ds,org_post_mst org, cmn_lookup_mst cmn" );
			sb.append("  , HR_PAY_POST_PSR_MPG p ");
		//}
		//sb.append("  where pd.loc_id = '" + locId +"' and P.POST_ID = PD.POST_ID and  pd.dsgn_id = ds.dsgn_id and ds.lang_id = 1  " );
		sb.append("  where pd.loc_id in (" + locId +") and P.POST_ID = PD.POST_ID and  pd.dsgn_id = ds.dsgn_id and ds.lang_id = 1  " );
		...
		public List getPostNameForDisplayThruEmpId(String locId, String empId, long langId)

	{
		logger.info("Inside getPostNameForDisplayThruEmpId");
		List postNameList = new ArrayList();
		Session hibSession = getSession();
		StringBuffer sb = new StringBuffer();
		...
		sb.append(" and o.emp_id = "+ empId +" and  pd.loc_id in ( " +locId + " ) and pd.dsgn_id = ds.dsgn_id and ds.lang_id = " + langId + " order by pd.post_name ");
		logger.info("The Query String is:-"+sb.toString());
	    Query query = hibSession.createSQLQuery(sb.toString());
	    logger.info("query getPostNameForDisplayThruEmpId :-"+query.toString());
		...
		public String getNetSalary(String sevaarthId){
			List sevaarthIdList = new ArrayList();
			Session hibSession = getSession();
			StringBuffer sb = new StringBuffer();
			sb.append("SELECT NET_TOTAL FROM HR_PAY_PAYBILL where EMP_ID in (SELECT EMP_ID FROM HR_EIS_EMP_MST where SEVARTH_EMP_CD = '");
			sb.append(sevaarthId);
			sb.append("' and PAYBILL_GRP_ID  in (SELECT max(PAYBILL_GRP_ID) FROM HR_PAY_PAYBILL where EMP_ID in (SELECT EMP_ID FROM HR_EIS_EMP_MST where SEVARTH_EMP_CD = '");
			sb.append(sevaarthId);
			sb.append("')  and PAYBILL_MONTH  in (SELECT max(PAYBILL_MONTH) FROM HR_PAY_PAYBILL where EMP_ID in (SELECT EMP_ID FROM HR_EIS_EMP_MST where SEVARTH_EMP_CD = '");
			sb.append(sevaarthId);
			sb.append("'))))");
			logger.info("salary query.."+sb);
			Query query = hibSession.createSQLQuery(sb.toString());
			sevaarthIdList = query.list();
		    logger.info("salary id  List size is:-"+sevaarthIdList.size());
		    //logger.info("sevaarth id List size is:-"+sevaarthIdList.listIterator());
		    String netSalary ="";
		    if( sevaarthIdList.size() >0)		    
		     netSalary = sevaarthIdList.get(0).toString();
			return netSalary;
		}
		...
		public List getAllUserPostRltDatabyDesgNew(long loc_id,long desgId)
		{
			...
			sb.append("left join HR_PAY_OFFICEPOST_MPG d on d.POST_ID=a.POST_ID ");
			sb.append("inner join MST_DCPS_DDO_OFFICE e on e.DCPS_DDO_OFFICE_MST_ID=d.OFFICE_ID ");
			sb.append("where b.LOC_ID = '"+loc_id+"' and c.POST_ID is null and a.LOCATION_CODE = '"+loc_id+"' and (a.END_DATE > '2012-08-15' or a.END_DATE is null) ");
	...
	*/
			...
			/*sb.append("select pd.post_id,  pd.post_name || sub.SUBJECT_NAME ");
			sb.append(" from org_post_details_rlt pd , FRM_SUB_POSTID_MPG sub , org_post_mst mst where mst.POST_ID = pd.POST_ID and mst.ACTIVATE_FLAG = 1  and mst.DSGN_CODE = " + desgId + " and  pd.POST_ID=sub.POST_ID and ");	   
			sb.append(" mst.LOCATION_CODE=  '"+ loc_id + "' and pd.lang_id = 1 and ");
		    sb.append(" pd.post_id not in (select prlt.post_id from org_userpost_rlt prlt,org_post_details_rlt pd1 where pd1.post_id = prlt.post_Id and (prlt.end_date is  null or prlt.activate_flag = 1) and pd1.loc_id="+loc_id+")  ");
		    //added by shailesh
			...
			sb.append(" and pd.post_id not in(select post_id from rlt_dcps_payroll_emp RL,mst_dcps_emp EM where RL.dcps_Emp_Id = EM.dcps_Emp_Id and RL.post_Id is not null and EM.ddo_Code = '"+ddoCode+"')");
		    sb.append("  order by pd.post_name  ");*/
			
			//added by abhishek pratap
			sb.append("select a.POST_ID,b.post_name || sub.SUBJECT_NAME from ");
			sb.append("org_post_mst a inner join ORG_POST_DETAILS_RLT b on b.POST_ID=a.POST_ID ");
			sb.append("left outer join ORG_USERPOST_RLT c on c.POST_ID=a.POST_ID and c.ACTIVATE_FLAG=1 ");
			sb.append("inner join HR_PAY_OFFICEPOST_MPG d on d.POST_ID=a.POST_ID ");
			//sb.append("inner join HR_PAY_POST_PSR_MPG h on h.POST_ID=a.POST_ID ");
			sb.append("inner join MST_DCPS_DDO_OFFICE e on e.DCPS_DDO_OFFICE_MST_ID=d.OFFICE_ID ");
			sb.append("inner join FRM_SUB_POSTID_MPG sub on b.POST_ID=sub.POST_ID ");
			sb.append("where b.LOC_ID = '"+loc_id+"' and c.POST_ID is null and a.LOCATION_CODE = '"+loc_id+"' and (a.END_DATE > '2013-04-15' or a.END_DATE is null) ");
			sb.append("and d.post_Id not in (select RL.post_Id from rlt_dcps_payroll_emp RL,mst_dcps_emp EM where RL.dcps_Emp_Id = EM.dcps_Emp_Id and RL.post_Id is not null and EM.ddo_Code =(select ddo_code from org_ddo_mst where location_code= '"+loc_id+"' )and EM.reg_Status not in (1,2)) ");
			sb.append("and b.DSGN_ID = "+desgId );
			sb.append(" and a.ACTIVATE_FLAG = 1  and POST_TYPE_LOOKUP_ID  in  (10001198130,10001198129,10001198155) order by b.post_name");

			logger.info( "execyting Query......."+sb.toString());

			Query query = hibSession.createSQLQuery(sb.toString());
			logger.info("getAllUserPostRltDatabyDesgNew:-"+query.toString());
			userpostrlt = query.list();
			logger.info("List size is:-"+userpostrlt.size());
	...
	/*	public List getAllUserPostRltDatabyDesgNew(long loc_id,long desgId)
		{
			...
			/*sb.append("  select pd.post_id,  pd.post_name ");
			sb.append(" from org_post_details_rlt pd  where pd.dsgn_id = " + desgId + " and   ");	   
			sb.append(" pd.loc_id =  "+ loc_id + " and pd.lang_id = 1 and ");
		    sb.append(" pd.post_id not in (select prlt.post_id from org_userpost_rlt prlt,org_post_details_rlt pd1 where pd1.post_id = prlt.post_Id and (prlt.end_date is  null or prlt.activate_flag = 1) and pd1.loc_id="+loc_id+")  ");
		    sb.append("  order by pd.post_name  ");*/
		    
			/*sb.append("  select pd.post_id,  pd.post_name, sb.subject_name ");
			sb.append(" from org_post_details_rlt pd, FRM_SUB_POSTID_MPG sb  where pd.dsgn_id = " + desgId + " and pd.POST_ID=sb.POST_ID and");	   
			sb.append(" pd.lang_id = 1 and");
		    sb.append(" pd.post_id not in (select prlt.post_id from org_userpost_rlt prlt,org_post_details_rlt pd1 where pd1.post_id = prlt.post_Id and (prlt.end_date is  null or prlt.activate_flag = 1) and pd1.loc_id="+loc_id+")");
		    sb.append(" order by pd.post_name  ");
		    logger.info( "execyting Query......."+sb.toString());
		...
		public List getPostNameForDisplayWithSearch(long locId,String lPostName,String PsrNo,String BillNo,String Dsgn )
		{
			...
			Session hibSession = getSession();
			StringBuffer sb = new StringBuffer();
			logger.info("hi i am in getting post with search lPostName"+lPostName);
			logger.info("hi i am in getting post with search PsrNo"+PsrNo);
			logger.info("hi i am in getting post with search BillNo"+BillNo);
			logger.info("hi i am in getting post with search Dsgn"+Dsgn);
		...
		public List getSubLocationDDOs(String postId){
			...
			//sb.append("SELECT ddo.LOCATION_CODE FROM RLT_ZP_DDO_MAP rep, ORG_DDO_MST ddo where ddo.DDO_CODE = rep.ZP_DDO_CODE and rep.FINAL_DDO_POST_ID="+postId);
			//end by roshan
			
			logger.info("---- getSubDDOs DAo---"+sb);
			Query query = session.createSQLQuery(sb.toString());
			ddoDtls = query.list();
			logger.info("ddoDtls is "+ddoDtls);
			return ddoDtls;
		}
		...
		public List getSvnPcData(String gradeId) {
			...
			logger.error("PARVEZ AND ASHISH NIGHT SHIFT :: "+queryBuffer.toString());
		...
		public String getLevelId(String strCell) {
			List basicLst = new ArrayList();
			Session hibSession = getSession();
			StringBuffer sb = new StringBuffer();
			sb.append("SELECT id FROM RLT_PAYBAND_GP_STATE_7PC where LEVEL_ID ='"+strCell+"'");
			logger.info("salary query.."+sb);
			Query query = hibSession.createSQLQuery(sb.toString());
			basicLst = query.list();
		    logger.info("salary id  List size is:-"+basicLst.size());
		    //logger.info("sevaarth id List size is:-"+sevaarthIdList.listIterator());
		    String levelId ="";
		    if( basicLst.size() >0)		    
		    	levelId = basicLst.get(0).toString();
			return levelId;
		}
